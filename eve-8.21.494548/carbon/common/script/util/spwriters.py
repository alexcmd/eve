#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/carbon/common/script/util/spwriters.py
import types
import htmlwriter
import util
import blue
...
import iocp
import re
PETITION_LINK_NOTE_PATTERN = '\\b(?=\\w)pet: ?(\\d+)\\b(?!\\w)'
PETITION_LINK_NOTE_REPLACE = '<a href="/gm/petitionClient.py?action=ViewPetition&petitionID=\\1">pet: \\1</a>'
...
class SPHtmlWriter(htmlwriter.HtmlWriterEx):
    ...
    def __init__(self, template = 'script:/wwwroot/lib/template/base.html', menu = '', page = '', showMenu = True):
        ...
        if macho.mode == 'server':
            ...
            self.AddTopMenuSub('INFO', 'StacklessIO', '/info/stacklessio.py', disabled=dPROG)
            self.AddTopMenuSub('INFO', 'MachoNet', '/info/machoNet.py', disabled=dPROG)
            self.AddTopMenuSub('INFO', 'Crest', '/info/cow.py', disabled=dPROG)
            self.AddTopMenuSub('INFO', 'OAuth2', '/info/oauth2.py')
            self.AddTopMenu('', '', '', '')
        ...
        if macho.mode == 'server':
            ...
            if menu == 'INFO':
                ...
                self.AddMenu('StacklessIO', 'StacklessIO', '', '/info/stacklessio.py')
                self.AddMenu('MachoNet', 'MachoNet', '', '/info/machoNet.py')
                self.AddMenu('Crest', 'Crest', '', '/info/cow.py')
            elif menu == 'ADMIN':
                self.AddMenu('Counters', 'Counters', '', '/admin/counters.py')
                self.AddMenu('Errors', 'Errors', '', '/admin/errors.py')
                self.AddMenu('Logs', 'Logs', '', '/admin/logs.py')
    ...
    if macho.mode == 'server':
        ...
        def WriteMetricGraph(self, counterID, rows = 50, graph = 'area', width = 1000, height = 300, data = False):
            if rows > 5000:
                self.WriteError('Max 5000 rows supported')
                return
            c = self.cache.Index(const.cacheMetricCounters, counterID)
            if c.counterType == 'D':
                rs = self.DB2.SQLInt('TOP %d counterDate, value' % rows, 'zmetric.dateCounters', '', 'counterDate DESC', 'counterID', counterID)
            elif c.counterType == 'T':
                rs = self.DB2.SQLInt('TOP %d counterDate, value' % rows, 'zmetric.timeCounters', '', 'counterDate DESC', 'counterID', counterID)
            elif c.counterType == 'S':
        ...
        def MultiNotes(self, typeID, referenceID, title, redir, noteID = None, rows = 300):
            ...
            for r in rs:
                ...
                lines.append([self.Font(re.sub(PETITION_LINK_NOTE_PATTERN, PETITION_LINK_NOTE_REPLACE, r.note).replace('\n', '<br>'), size=2)])
        ...
        def UserTags(self, userID):
            rs = self.dbzuser.UserTags_Select(userID)
            s = ''
            for r in rs:
                if s != '':
                    s += self.MidDot(4)
        ...
        def CountryFlag(self, countryName, flagSize = '16'):
            return self.Image('/img/flags/%(s)sx%(s)s/flag_%(c)s.png' % {'s': flagSize,
             'c': countryName}, 'border=0 align=absmiddle')
        ...
        def FSDField(self, field):
            return field + " <a href='http://carbon/wiki/FSD' >" + "<img src='/img/FSDIcon.png' title='This data is coming from FSD and should be authored there' height='18px' width='18px'/>" + '</a>'
        ...
        def IPAddress(self, ipAddress, ipAddressID = None):
            if ipAddressID:
                return self.Link('/gm/users.py', ipAddress, {'action': 'IPLogs',
                 'IPID': ipAddressID})
            if ipAddress:
                return self.Link('/gm/users.py', ipAddress, {'action': 'IPLogs',
                 'IP': ipAddress})
