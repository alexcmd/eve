#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/carbon/common/script/util/htmlwriter.py
import random
import blue
import urllib
import urllib2
...
def GetMessageEnglishByID(messageID, defaultValue):
    ...
    if messageID is not None and messageID != '':
        message = localization.Message.Get(messageID)
...
exports = {'htmlwriter.UnpackModalDialogueResult': UnpackModalDialogueResult,
 ...
 'htmlwrite.CheckValue': CheckValue}
...
class HtmlWriter():
    ...
    def LogError(self, *args, **keywords):
        self.Log(log.LGERROR, *args, **keywords)
    ...
    def Generate(self):
        ...
        if macho.mode == 'server':
            ...
            nodeInfo = '&nbsp;&middot;&nbsp; Node: %s' % self.Link('/admin/network.py', '<span clsss="gray">%s</span>' % nodeID, {'action': 'Node',
             'nodeID': nodeID})
        ...
        template = self.baseTemplate.split('/')[-1]
        return TemplateRender(template, **self.inserts)
...
class Form():
    ...
    def AddCheckbox(self, name, text = '', checked = 0, prettyName = None, disabled = 0):
        if prettyName is None:
            prettyName = name
        self.elements.append(['<label class="form-padding-label" for="%s">%s</label>' % (name, prettyName), self.Checkbox(name, text, checked, prettyName, disabled), '<label class="form-label" for="%s">%s</label>' % (name, text)])
