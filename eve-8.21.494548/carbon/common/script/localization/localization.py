...
class Localization(object):
    ...
    def LoadLanguageData(self):
        if boot.role == 'client':
            picklePrefixes = [self.CLIENT_PREFIX]
        else:
            picklePrefixes = ['res:/localization/localization_', 'res:/localization/localization_shared_', 'res:/localization/localization_dustui_']
        supportedLanguagesByPrefix = {}
        for prefix in picklePrefixes:
            supportedLanguagesByPrefix[prefix] = self._ReadLocalizationMainPickle(prefix + 'main' + self.PICKLE_EXT)
        ...
        for prefix in picklePrefixes:
            if not self._ReadLocalizationLanguagePickles(prefix, supportedLanguagesByPrefix[prefix]):
    ...
    def LoadPrimaryLanguage(self, prefix, languageID = None, importantOnly = False):
        ...
        if self._primaryLanguageID in self.languagesDefined:
            if not self._LoadLanguagePickle(prefix, self._primaryLanguageID, importantOnly):
                return False
        else:
    ...
    def LoadSecondaryLanguageForClient(self, languageID):
        ...
        if self._secondaryLanguageID in self.languagesDefined:
            if not self._LoadLanguagePickle(self.CLIENT_PREFIX, self._secondaryLanguageID):
                return False
        else:
            LogError("Language '", languageID, "' is not enabled. Text data was not loaded.")
            return False
    ...
    def _ReadMainLocalizationData(self, unPickledObject):
        ...
        if self.FIELD_LANGUAGES in unPickledObject:
            self.languagesDefined.update(dict(filter(lambda (k, v): self._IsValidLanguage(k), unPickledObject[self.FIELD_LANGUAGES].items())))
        else:
        ...
        if self.FIELD_MAX_REVISION in unPickledObject:
            ...
            return []
        return unPickledObject[self.FIELD_LANGUAGES].keys()
    ...
    def _IsValidLanguage(self, languageID):
        if boot.role == 'client':
            if languageID == localization.LOCALE_SHORT_CHINESE:
                ...
                return boot.region != 'optic' and languageID not in ('fr', 'it', 'es')
        return True
    ...
    @telemetry.ZONE_METHOD
    def _ReadLocalizationMainPickle(self, pickleName):
        unPickledObject = self._LoadPickleData(pickleName)
        if unPickledObject is None:
            return []
        supportedLanguages = self._ReadMainLocalizationData(unPickledObject)
    ...
    def _ReadLocalizationLanguagePickles(self, prefix, supportedLanguages):
        ...
        if boot.role == 'client':
            ...
            if prefsLanguage != primaryLanguage:
                if not (self.LoadPrimaryLanguage(prefix, importantOnly=False) and self.LoadSecondaryLanguageForClient(prefsLanguage)):
                    return False
            else:
            ...
            for aLangCode in supportedLanguages:
                if aLangCode != self._primaryLanguageID and not self._LoadLanguagePickle(prefix, aLangCode):
                    return False

        return True

    @telemetry.ZONE_METHOD
    def _LoadLanguagePickle(self, prefix, languageID, importantOnly = False):
        unPickledObject = self._LoadPickleData(prefix + languageID + self.PICKLE_EXT)
    ...
    def _LoadPickleData(self, pickleName):
        pickleFile = miscUtil.GetCommonResource(pickleName)
        if not pickleFile:
...
exports = {'localization.GetByMessageID': LOCALIZATION_REF.GetByMessageID,
 ...
 'localization.LoadLanguageData': LOCALIZATION_REF.LoadLanguageData,
 'localization.LoadPrimaryLanguage': LOCALIZATION_REF.LoadPrimaryLanguage,
 'localization.LoadSecondaryLanguageForClient': LOCALIZATION_REF.LoadSecondaryLanguageForClient,
