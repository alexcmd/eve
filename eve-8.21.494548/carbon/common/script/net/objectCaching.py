#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/carbon/common/script/net/objectCaching.py
import blue
import telemetry
import uthread
import types
...
class CoreObjectCachingSvc(service.Service):
    ...
    def LoadCache(self, ipaddress):
        ...
        blue.pyos.SpyDirectory(directory, self.__OnMethodCallCachingDetailsChanged)
        self.LoadDirectory(directory, 'methodCallCachingDetails')
        directory = self.cachePath + 'CachedMethodCalls'
        blue.pyos.SpyDirectory(directory, self.__OnCachedMethodCallsChanged)
        self.LoadDirectory(directory, 'cachedMethodCalls')
        directory = self.cachePath + 'CachedObjects'
        blue.pyos.SpyDirectory(directory, self.__OnCachedObjectsChanged)
        try:
            self.LoadDirectory(blue.paths.ResolvePath(u'app:/') + 'bulkdata', 'cachedObjects')
        ...
        for filename in os.listdir(directory):
            ...
            try:
                ...
                if not updateOnly:
                    ...
                    key = self.__KeyFromFileName(filename)
                ...
                if load:
                    ...
                    if fileData:
                        try:
                            ...
                            self.LogError('Failed to load data from ', filename, ', fnkey=', self.__KeyFromFileName(filename))
                            log.LogTraceback()
                            raise 
