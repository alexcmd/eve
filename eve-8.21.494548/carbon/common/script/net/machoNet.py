#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/carbon/common/script/net/machoNet.py
import blue
import uthread
import stackless
import sys
...
class MachoNetService(service.Service):
    ...
    def SendMessageToPlayers(self, message, includeEve, includeDust):
        recipients = []
        for sid, sess in base.allSessionsBySID.iteritems():
            userID = getattr(sess, 'userid', None)
            clientID = getattr(sess, 'clientID', None)
            if userID and clientID:
                if util.IsDustUser(userID) and includeDust or util.IsEveUser(userID) and includeEve:
    ...
    def ForwardCharacterNotification(self, charid, method, *args):
        callTimer = base.CallTimer('ForwardCharacterNotification::%s (Broadcast\\Client)' % method)
        try:
            clientIDs = [ sess.clientID for sess in base.FindSessions('charid', [charid]) ]
            if len(clientIDs):
                ...
                if nodeID == self.GetNodeID():
                    self.LogInfo('Character ', charid, " was not reachable, so he didn't get this notification.")
                    return
                ...
                try:
                    ...
                    try:
                        ...
                        if e.reason == 'Connected transport closed':
                            ...
                            if e.reasonCode in gpcs.TRANSPORT_CLOSED_MESSAGES:
    ...
    def GetMetrics(self):
        kv = util.KeyVal({'CallsReceived': {},
         'CallsSent': {},
         'NotificationsReceived': {},
         'NotificationsSent': {}})
        for functionName, receivedCount in gpcs.CoreServiceCall.__recvServiceCallCount__.iteritems():
            kv.CallsReceived[functionName] = receivedCount

        for functionName, sentCount in gpcs.CoreServiceCall.__sendServiceCallCount__.iteritems():
            kv.CallsSent[functionName] = sentCount

        for functionName, receivedCount in gpcs.CoreBroadcastStuff.__notifyRecvCount__.iteritems():
            kv.NotificationsReceived[functionName] = receivedCount

        for functionName, sentCount in gpcs.CoreServiceCall.__sendNotifyCount__.iteritems():
    ...
    def GracefulShutdown(self, userID, when, duration, explanationLabel):
        if when:
            self.LogNotice('Graceful Shutdown requested by ', userID, ' to occur at ', util.FmtDateEng(when), ', duration ', duration or 0, ' minutes, explanation: ', explanationLabel)
            if when < blue.os.GetWallclockTime() + const.HOUR:
                notify = blue.os.GetWallclockTime()
            else:
                notify = when - const.HOUR
            self.shutdown = util.KeyVal(notify=notify, userID=userID, when=when, duration=duration, explanationLabel=explanationLabel)
        else:
    ...
    def __PerformGracefulShutdown(self):
        ...
        try:
            ...
            if counterDate:
                self.dbLog.LogProcessClusterShutdownStarted('machoNet')
                ...
                for functionName, receivedCount in summaryReceived.iteritems():
                    dbzmetric.ColumnCounters_UpdateMulti(30021, 'D', counterDate, 2000000004, None, unicode(functionName), receivedCount)

                for functionName, sentCount in summarySent.iteritems():
                    dbzmetric.ColumnCounters_UpdateMulti(30022, 'D', counterDate, 2000000004, None, unicode(functionName), sentCount)
                ...
                for functionName, receivedCount in summaryReceived.iteritems():
                    dbzmetric.ColumnCounters_UpdateMulti(30023, 'D', counterDate, 2000000004, None, unicode(functionName), receivedCount)

                for functionName, sentCount in summarySent.iteritems():
                    dbzmetric.ColumnCounters_UpdateMulti(30024, 'D', counterDate, 2000000004, None, unicode(functionName), sentCount)

                self.dbLog.LogProcessClusterShutdownCompleted('machoNet', int_1=nodeCount, int_2=proxyCount, int_3=exceptionCount)
        except Exception:
    ...
    def __PrimeVIPList(self):
        ...
        for r in self.session.ConnectToSolServerService('DB2').CallProc('zuser.Users_SelectByRole', ROLE_VIPLOGIN):
            self.vipkeys.add(macho.CryptoHash(util.CaseFold(r.userName)))
    ...
    def IsThisDudeCoolForLogin(self, vipKey):
        if self.eveClientAppID is not None:
            isEveInVip = self.IsAppInVIP(self.eveClientAppID)
        else:
            isEveInVip = self.vipMode
        if isEveInVip:
            return self.__IsVIP(vipKey)
        return True
    ...
    def Authenticate(self, transport, request, sessionID):
        ...
        ret = {'sessionID': sessionID,
         ...
         'session_init': self.session.ConnectToRemoteService('authentication').Login(sessionID, request['user_name'], request['user_password'], request['user_password_hash'], const.userConnectTypeClient, transport.address, clientID, request, ssoToken=ssoToken)}
        ...
        if len(destAddress.narrowcast):
            ...
            if not done:
                if len(destAddress.narrowcast) == 1 and (idtype, destAddress.narrowcast[0]) in self.spam:
                    ...
                    if self.transportIDbyProxyNodeID:
                        spam, clientIDs, notfound = base.FindClientsAndHoles(idtype, destAddress.narrowcast, len(self.transportIDbyProxyNodeID) * 2)
                    else:
                        spam, clientIDs, notfound = base.FindClientsAndHoles(idtype, destAddress.narrowcast, 20)
                else:
                    spam, clientIDs, notfound = base.FindClientsAndHoles(idtype, destAddress.narrowcast, None)
        ...
        try:
            ...
            while not transport.IsClosed():
                ...
                try:
                    ...
                    if llv:
                        llvresponse = transport.HandleClientAuthentication(self.GetClusterSessionCounts('EVE:Online')[0], transportID, counter, sessionID)
                        counter += 1
                    ...
                    if self.stop:
                        return
                    if llv:
                        machoTransport = macho.StreamingHTTPMachoTransport(transportID, transport, transname, self)
                        self.transportsByID[transportID] = machoTransport
                        ...
                        s = base.CreateSession(sessionID, const.session.SESSION_TYPE_GAME)
        ...
        try:
            while 1:
                ...
                try:
                    ...
                    if theMessage.command == const.cluster.MACHONETMSG_TYPE_SESSIONCHANGENOTIFICATION and macho.mode == 'proxy':
                        ...
                        if len(destTransports) == 1:
                            ...
                            if srcTransport.transportName == 'tcp:packet:machoNet':
                                ...
                                theMessage.source = macho.MachoAddress(clientID=destTransport.clientID)
                    ...
                    srcTransport.Write(theMessage.ErrorResponse(const.cluster.MACHONETERR_WRAPPEDEXCEPTION, (macho.DumpsSanitized(RuntimeError(e.payload)),)))
                    sys.exc_clear()
                except UnMachoDestination as e:
    ...
    def StatusSocketLoop(self):
        try:
            ...
            log.LogException('Status socket threw and exception, and is not stopping')
