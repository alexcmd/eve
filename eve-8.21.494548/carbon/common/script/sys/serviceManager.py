#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/carbon/common/script/sys/serviceManager.py
...
class ServiceManager(log.LogMixin):
    ...
    def GetService(self, serviceName, ms = None):
        ...
        if srv and srv.state == service.SERVICE_RUNNING:
            return srv
        if serviceName in self.services:
            if self.services[serviceName].state in (service.SERVICE_START_PENDING, service.SERVICE_STARTING_DEPENDENCIES) and '/thinclient' not in blue.pyos.GetArg():
    ...
    def GetLogName(self, thingie):
        from macho import GetLogName as GetMachoLogName
