#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/carbon/common/script/sys/locks.py
import blue
import sys
import stackless
...
def LockCycleReport(graph = None, out = None, timeLimit = None, pathLimit = 10):
    ...
    def PathTime(p):
        return sum((getattrg[n]'time'0 for n in p))
    ...
    for v in players[0:maximum]:
        k = map[v]
        print >> out, ' %3s = %r %s' % (k, v, '(tasklet_id %s)' % v.tasklet_id if stackless and isinstance(v, stackless.tasklet) else '')
