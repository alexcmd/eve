...
class Config():
    ...
    def GotInitData(self, initdata):
        ...
        if boot.GetValue('useFSD', 1) == 1:
            graphicFile = 'res:/staticdata/graphicIDs.static'
            graphicSchema = 'res:/staticdata/graphicIDs.schema'
            iconFile = 'res:/staticdata/iconIDs.static'
            iconSchema = 'res:/staticdata/iconIDs.schema'
            soundFile = 'res:/staticdata/soundIDs.static'
            soundSchema = 'res:/staticdata/soundIDs.schema'
            self.LogNotice('Loading graphicIDs from file')
            self.graphics = self.LoadFSDData(graphicFile, graphicSchema, idCacheCount=100)
            self.LogNotice('Loading iconIDs from file')
            self.icons = self.LoadFSDData(iconFile, iconSchema, idCacheCount=100)
            self.LogNotice('Loading soundIDs from file')
            self.sounds = self.LoadFSDData(soundFile, soundSchema, idCacheCount=100)
        else:
