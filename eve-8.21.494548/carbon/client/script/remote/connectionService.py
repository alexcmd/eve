#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/carbon/client/script/remote/connectionService.py
...
class ConnectionService(Service):
    ...
    def ConnectSso(self, host, port, token, ct):
        try:
            self.LogInfo('calling connect')
            address = '%s:%d' % (str(host), port)
            response = self.machoNet.ConnectToServer(address, 'fakeUser', 'fakePW', ct, token)
            self.LogInfo('connect completed...')
        except GPSException as what:
            self.LogError('connect failed %s' % what)
            raise UserError('LoginConnectFailed', {'what': what})
    ...
    def LoginSso(self, token):
        ...
        try:
            ...
            response = self.ConnectSso(util.GetServerName(), util.GetServerPort(), token, 'tcp')
            return response
        finally:
            self.reentrancyGaurd = 0
