#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/carbon/client/script/ui/control/editPlainText.py
import util
import blue
import telemetry
import base
...
class EditPlainTextCore(uicls.Scroll):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self._hintLabel = uicls.Label(parent=self.sr.clipper, align=uiconst.TOTOP, padding=6, state=uiconst.UI_DISABLED)
    ...
    def ShowAttributePanel(self):
        if not self.readonly:
            self.sr.attribPanel = uicls.FontAttribPanel(align=uiconst.TOTOP, pos=(0, 0, 0, 22), parent=self, idx=0, state=uiconst.UI_PICKCHILDREN, counterMax=self.counterMax, countWithTags=self.countWithTags)
    ...
    def SetValue(self, text, scrolltotop = 0, cursorPos = None, preformatted = 0, html = 1, fontColor = None):
        text = text or ''
        if text and not self.EnoughRoomFor(len(uiutil.StripTags(text))):
            return
        self.GetAbsolute()
        self._activeParams = None
    ...
    def Insert(self, char):
        if char not in (uiconst.VK_BACK, 127) and not self.EnoughRoomFor(1):
            uicore.Message('uiwarning03')
            return
        node = self.GetActiveNode()
        if node is None:
    ...
    def InsertToGlyphString(self, glyphString, params, text, idx):
        if params.url:
            uicls.BaseLink().FormatLinkParams(params, linkStyle=self.linkStyle)
    ...
    def DrawLine(self, glyphString, buf, bx0, by0, startIdx, endIdx, createLinks = True):
        ...
        for t in glyphString[startIdx:endIdx]:
            ...
            if params.url:
                ...
                params = params.Copy()
                uicls.BaseLink().FormatLinkParams(params, linkState, linkStyle=self.sr.node.scroll.linkStyle)
