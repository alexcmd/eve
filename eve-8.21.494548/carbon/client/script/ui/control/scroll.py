#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/carbon/client/script/ui/control/scroll.py
import blue
import telemetry
import uthread
import sys
...
class ScrollCore(uicls.Container):
    ...
    def OnGlobalMouseHover(self, obj, *args):
        if uicore.IsDragging() and (obj == self or uiutil.IsUnder(obj, self.sr.content)):
    ...
    def Prepare_ActiveFrame_(self):
        self.sr.activeframe = uicls.Frame(name='__activeframe', color=(1.0,
         1.0,
         1.0,
         uiconst.ACTIVE_FRAME_ALPHA), frameConst=uiconst.FRAME_BORDER1_SHADOW_CORNER0, parent=self, state=uiconst.UI_HIDDEN, idx=0)
    ...
    def GetHeaderMenu(self, label):
        m = []
        if self.smartSort:
            m += [(uiutil.MenuLabel('/Carbon/UI/Commands/CmdMakePrimary'), self.MakePrimary, (label,))]
        if self.smartSort or self.allowFilterColumns:
            if len(self.GetColumns()) > 1:
                m += [(uiutil.MenuLabel('/Carbon/UI/Common/Hide', {'label': label}), self.HideColumn, (label,))]
    ...
    def GetShowColumnMenu(self):
        ...
        for label in self.sr.headers:
            if label not in self.GetColumns():
                m.append((uiutil.MenuLabel('/Carbon/UI/Common/Show', {'label': label}), self.ShowColumn, (label,)))
    ...
    def Copy(self, *args):
        ...
        strippedText = uiutil.StripTags(allLabels)
        if strippedText:
...
class ScrollHandleCore(uicls.Container):
    ...
    def OnMouseEnter(self, *args):
        if self.sr.hilite:
            uicore.animations.FadeIn(self.sr.hilite, 0.5, duration=0.2)

    def OnMouseExit(self, *args):
        if self.sr.hilite:
            uicore.animations.FadeOut(self.sr.hilite, duration=0.2)
