...
class SinglelineEditCore(uicls.Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self._textClipper = uicls.Container(name='_textClipper', parent=self, clipChildren=True, padding=(1, 0, 1, 0))
    ...
    def Prepare_(self):
        ...
        self.sr.text = uicls.Label(text='', parent=self._textClipper, name='value', align=uiconst.CENTERLEFT, state=uiconst.UI_DISABLED, maxLines=1, left=self.TEXTLEFTMARGIN, fontStyle=self.fontStyle, fontFamily=self.fontFamily, fontPath=self.fontPath, fontsize=self.fontsize)
        self.sr.hinttext = uicls.Label(text='', parent=self._textClipper, name='hinttext', align=uiconst.CENTERLEFT, state=uiconst.UI_DISABLED, maxLines=1, left=self.TEXTLEFTMARGIN, fontStyle=self.fontStyle, fontFamily=self.fontFamily, fontPath=self.fontPath, fontsize=self.fontsize)
    ...
    def Prepare_Background_(self):
        if not self.sr.underlay:
            self.sr.underlay = uicls.Frame(name='__underlay', frameConst=('ui_1_16_161', 7, -2), parent=self)

    def Prepare_ActiveFrame_(self):
        self.sr.activeframe = uicls.Frame(name='__activeframe', parent=self, align=uiconst.TOALL, state=uiconst.UI_HIDDEN, idx=1, color=(1.0,
         1.0,
         1.0,
         uiconst.ACTIVE_FRAME_ALPHA))

    def Prepare_Caret_(self):
        self.sr.caret = uicls.Fill(parent=self._textClipper, name='caret', align=uiconst.TOPLEFT, color=(1.0, 1.0, 1.0, 0.75), pos=(self.TEXTLEFTMARGIN,
         1,
         1,
    ...
    def AddIconButton(self, texturePath, hint = None):
        rightAlignedButton = uicls.ButtonIcon(texturePath=texturePath, pos=(0, 0, 16, 16), align=uiconst.CENTERRIGHT, parent=self, hint=hint, idx=0)
    ...
    def LoadCombo(self, id, options, callback = None, setvalue = None, comboIsTabStop = 1):
        ...
        combo = uicls.Combo(parent=self, label='', options=options, name=id, select=setvalue, callback=self.OnComboChange, pos=(0, 0, 16, 16), align=uiconst.BOTTOMRIGHT)
    ...
    def GetSelectionLayer(self):
        if not self.sr.selection:
            w, h = self.GetAbsoluteSize()
            self.sr.selection = uicls.Fill(parent=self._textClipper, name='selection', align=uiconst.TOPLEFT, pos=(0,
             1,
             0,
             h - 2), idx=1)
