#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/carbon/client/script/ui/control/edit.py
import util
import blue
import base
import uthread
...
class EditCore(parser.ParserBase, uicls.Scroll):
    ...
    def GetMenuDelegate(self, node = None):
        ...
        m.append((uiutil.MenuLabel('/Carbon/UI/Controls/Common/CopyAll'), self.CopyAll))
        if self.HasSelection():
            m.append((uiutil.MenuLabel('/Carbon/UI/Controls/Common/CopySelected'), self.Copy))
            if not self.readonly:
                m.append((uiutil.MenuLabel('/Carbon/UI/Controls/Common/CutSelected'), self.Cut))
        clipboard = uiutil.GetClipboardData()
        if clipboard and not self.readonly:
            m.append((uiutil.MenuLabel('/Carbon/UI/Controls/Common/Paste'), self.Paste, (clipboard,)))
        if not self.readonly:
            m.append(None)
            linkmenu = [(uiutil.MenuLabel('/EVE/UI/Common/Character'), self.LinkCharacter),
             (uiutil.MenuLabel('/EVE/UI/Common/Corporation'), self.LinkCorp),
             (uiutil.MenuLabel('/EVE/UI/Common/LocationTypes/SolarSystem'), self.LinkSolarSystem),
             (uiutil.MenuLabel('/EVE/UI/Common/LocationTypes/Station'), self.LinkStation),
             (uiutil.MenuLabel('/Carbon/UI/Controls/Common/ItemType'), self.LinkItemType)]
            m.append((uiutil.MenuLabel('/Carbon/UI/Controls/Common/AutoLink'), linkmenu))
...
class FontAttribPanelCore(uicls.Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.sr.bold = uicls.Label(text=localization.GetByLabel('/Carbon/UI/Controls/EditRichText/BoldSymbol'), parent=self, pos=(6, 0, 12, 0), align=uiconst.TOPLEFT, fontsize=14, color=(1.0, 1.0, 1.0, 0.6), state=uiconst.UI_NORMAL)
        self.sr.italic = uicls.Label(text=localization.GetByLabel('/Carbon/UI/Controls/EditRichText/ItalicSymbol'), parent=self, pos=(18, 0, 12, 0), align=uiconst.TOPLEFT, fontsize=14, color=(1.0, 1.0, 1.0, 0.6), state=uiconst.UI_NORMAL)
        self.sr.underline = uicls.Label(text=localization.GetByLabel('/Carbon/UI/Controls/EditRichText/UnderlineSymbol'), parent=self, pos=(30, 0, 12, 0), align=uiconst.TOPLEFT, fontsize=14, color=(1.0, 1.0, 1.0, 0.6), state=uiconst.UI_NORMAL)
        ...
        self.sr.anchor = uicls.Label(text=localization.GetByLabel('/Carbon/UI/Controls/EditRichText/LinkSymbol'), parent=self, pos=(110, 3, 18, 16), color=(1.0, 1.0, 1.0, 0.6), align=uiconst.TOPLEFT, hint=localization.GetByLabel('/Carbon/UI/Controls/EditRichText/AddLink'), fontsize=9, state=uiconst.UI_NORMAL)
        self.sr.anchor.OnClick = self.AddAnchor
        self.sr.clearnote = uicls.Label(text=localization.GetByLabel('/Carbon/UI/Controls/EditRichText/ClearTextSymbol'), parent=self, pos=(130, 0, 12, 16), color=(1.0, 1.0, 1.0, 0.6), align=uiconst.TOPLEFT, hint=localization.GetByLabel('/Carbon/UI/Controls/EditRichText/ClearText'), fontsize=14, state=uiconst.UI_NORMAL)
        self.sr.clearnote.OnClick = self.ClearNote
        languageIndicator = sm.GetService('ime').GetLanguageIndicator()
        self.sr.langIndicator = uicls.Label(text=languageIndicator, parent=self, color=(1.0, 1.0, 1.0, 0.6), fontsize=12, width=14, height=16, align=uiconst.TOPRIGHT, state=uiconst.UI_DISABLED, uppercase=True)
        if self.counterMax is not None:
            self.charCounter = uicls.Label(text='0/%s' % self.counterMax, parent=self, color=(1.0, 1.0, 1.0, 0.6), fontsize=12, left=30, align=uiconst.TOPRIGHT, state=uiconst.UI_DISABLED)
        self.expanding = 0
        self.expanded = 0
    ...
    def UpdateCounter(self, *args):
        ...
        elif isinstance(parent, uicls.EditPlainTextCore):
            currentLen = len(parent.GetAllText())
        else:
            currentLen = len(uiutil.StripTags(parent.GetSelectedText(getAll=True)))
