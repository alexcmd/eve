#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/carbon/client/script/ui/control/scrollentries.py
import fontConst
import blue
import base
import trinity
...
class SE_ListGroupCore(SE_BaseClassCore):
    ...
    def GetMenu(self):
        ...
        if not node.get('showmenu', True):
            return m
        if not node.open:
            m += [(uiutil.MenuLabel('/Carbon/UI/Common/Expand'), self.Toggle, ())]
        else:
            m += [(uiutil.MenuLabel('/Carbon/UI/Common/Collapse'), self.Toggle, ())]
