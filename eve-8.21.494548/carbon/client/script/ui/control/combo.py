#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/carbon/client/script/ui/control/combo.py
import uiconst
import uiutil
import uicls
import uthread
...
class ComboCore(uicls.Container):
    ...
    def Prepare_ActiveFrame_(self):
        self.sr.activeframe = uicls.Frame(name='__activeframe', parent=self, align=uiconst.TOALL, state=uiconst.UI_HIDDEN, idx=0, color=(1.0,
         1.0,
         1.0,
    ...
    def AutoAdjustWidth_(self):
        ...
        if self.adjustWidth and self.entries and currentAlign not in (uiconst.TOTOP, uiconst.TOBOTTOM, uiconst.TOALL):
            ...
            self.width = max([ uicore.font.GetTextWidth(each[0], fontFamily=self.fontFamily, fontStyle=self.fontStyle, fontPath=self.fontPath, fontsize=self.fontsize) for each in self.entries ] + [20]) + arrowContainerWidth
