#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/carbon/client/script/ui/uilib.py
import uiconst
import uicls
import uiutil
import uthread
    ...
    def UpdateMouseOver(self, *args):
        ...
        for root in self.rootObjects:
            ...
            if root.renderTargetStep:
                ...
                triobj = root.PickObject(int(uicore.ScaleDpi(self.x)), int(uicore.ScaleDpi(self.y)))
            else:
            ...
            if triobj:
                ...
                if pyObject and not isinstance(pyObject, uicls.LayerCore):
                    break

        newMouseOver = pyObject or uicore.desktop
        currentMouseOver = self.GetMouseOver()
        if getattr(triobj, 'auxMouseover', None):
            newAuxMouseOverRO = weakref.ref(triobj.auxMouseover)
        else:
            newAuxMouseOverRO = None
        mouseCaptureItem = self.GetMouseCapture()
        self._mouseOver = weakref.ref(newMouseOver)
        if not mouseCaptureItem and (currentMouseOver is not newMouseOver or newAuxMouseOverRO is not self._auxMouseOverRO):
            self._auxMouseOverRO = newAuxMouseOverRO
            if self._hoverThread:
                self._hoverThread.kill()
            if currentMouseOver:
                if uicore.IsDragging() and isinstance(currentMouseOver, uicls.DragDropObject) and uicore.dragObject is not currentMouseOver:
            ...
            if newMouseOver:
                if uicore.IsDragging() and isinstance(newMouseOver, uicls.DragDropObject) and uicore.dragObject is not newMouseOver:
            ...
            if msgID == WM_MOUSEMOVE:
                    ...
                    if not self.exclusiveMouseFocusActive:
                        if getattr(mouseCaptureItem, 'expandOnLeft', 0) and not self.rightbtn and self._expandMenu == uiconst.MOUSELEFT and getattr(mouseCaptureItem, 'GetMenu', None):
                            ...
                            if abs(self.x - x) < 3 and abs(self.y - y) < 3:
                                uthread.new(menu.ShowMenu, mouseCaptureItem, self.GetAuxMouseOver())
                ...
                if self.exclusiveMouseFocusActive:
                    ...
                    if not self.leftbtn and self._expandMenu == uiconst.MOUSERIGHT and (getattr(currentMouseOver, 'GetMenu', None) or self._auxMouseOverRO is not None):
                        ...
                        if abs(self.x - x) < 3 and abs(self.y - y) < 3:
                            uthread.new(menu.ShowMenu, currentMouseOver, self.GetAuxMouseOver())
                    self._expandMenu = None
                ...
                if self.mouseOver:
                    ...
                    if not calledMethod:
                        focus = uicore.registry.GetFocus()
                        if focus and uiutil.GetWindowAbove(self.mouseOver) == uicore.registry.GetActive():
