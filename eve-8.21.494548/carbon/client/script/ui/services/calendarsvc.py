#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/carbon/client/script/ui/services/calendarsvc.py
import service
import blue
import const
import util
...
class CalendarSvc(service.Service):
    ...
    def GetEventMenu(self, eventInfo, myResponse = None, getJumpOption = True, *args):
        m = []
        m.append((uiutil.MenuLabel('/Carbon/UI/Calendar/ViewEvent'), self.OpenEventWnd, (eventInfo,)))
            ...
            if eventInfo.ownerID != session.charid:
                if eventInfo.flag in [const.calendarTagCorp, const.calendarTagAlliance]:
                    if eventInfo.ownerID in [session.corpid, session.allianceid] and session.corprole & const.corpRoleChatManager == const.corpRoleChatManager:
                        canDelete = 1
                        m.append((uiutil.MenuLabel('/Carbon/UI/Calendar/EditEvent'), self.OpenEditEventWnd, (eventInfo,)))
                ...
                if eventInfo.flag is not const.calendarTagCCP:
                    if myResponse != const.eventResponseAccepted:
                        m.append((uiutil.MenuLabel('/Carbon/UI/Calendar/Accept'), self.RespondToEvent, (eventInfo.eventID, eventInfo, const.eventResponseAccepted)))
                    if myResponse != const.eventResponseMaybe:
                        m.append((uiutil.MenuLabel('/Carbon/UI/Calendar/MaybeReply'), self.RespondToEvent, (eventInfo.eventID, eventInfo, const.eventResponseMaybe)))
                    if myResponse != const.eventResponseDeclined:
                        m.append((uiutil.MenuLabel('/Carbon/UI/Calendar/Decline'), self.RespondToEvent, (eventInfo.eventID, eventInfo, const.eventResponseDeclined)))
            elif eventInfo.flag == const.calendarTagPersonal:
                canDelete = 1
                m.append((uiutil.MenuLabel('/Carbon/UI/Calendar/EditEvent'), self.OpenEditEventWnd, (eventInfo,)))
        if getJumpOption:
            m.append(None)
            m.append((uiutil.MenuLabel('/Carbon/UI/Calendar/GotoDay'), self.JumpToDay, (eventInfo,)))
        if canDelete:
            m.append(None)
            m.append((uiutil.MenuLabel('/Carbon/UI/Calendar/DeleteEvent'), self.DeleteEvent, (eventInfo.eventID, eventInfo.ownerID)))
        return m
