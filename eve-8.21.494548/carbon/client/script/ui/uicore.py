#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/carbon/client/script/ui/uicore.py
import uthread
import blue
import localization
import log
...
class UICoreBase():
    ...
    def CheckHint(self):
        if uicore.uilib.selectedCursorType != uiconst.UICURSOR_NONE:
            if uicore.uilib.auxMouseOver and hasattr(uicore.uilib.auxMouseOver, 'GetAuxiliaryTooltip'):
                uicore.uilib.mouseOver.auxiliaryHint = uicore.uilib.auxMouseOver.GetAuxiliaryTooltip()
            else:
                uicore.uilib.mouseOver.auxiliaryHint = None
            uthread.new(self.UpdateHint, uicore.uilib.mouseOver)
    ...
    def _Message(self, msgkey, dict = None, buttons = None, suppress = None, prioritize = 0, ignoreNotFound = 0, default = None, modal = True):
        ...
        if not hasattr(self, 'desktop') or self.desktop is None:
            print "Some dude is trying to send a message with uicore.Message before  it's ready.", msgkey, dict, buttons, suppress
            try:
