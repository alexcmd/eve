...
class EveConfig(util.config):
    ...
    def GetMessageTypeAndText(self, key, paramDict, onNotFound = 'return', onDictMissing = 'error', languageID = None):
        ...
        return util.KeyVal(text=text, type=msg.messageType)
    ...
    def _GetContentForMissingMessage(self, key, paramDict, languageID):
        if key != 'ErrMessageNotFound':
            ...
            return util.KeyVal(text='Could not find message with key ' + key + '. This is most likely due to a missing or outdated 10000001.cache2 bulkdata file, or the message is missing.', title='Message not found', type='fatal', audio='', icon='', suppress=False)
    ...
    def GetItemVolume(self, item, qty = None, invmgr = None):
        if item.typeID == const.typePlasticWrap and item.singleton:
            ...
            if volume <= 0:
                raise RuntimeError('Volume of a plastic wrap should neve be zero or less')
        elif item.categoryID == const.categoryShip and not item.singleton and item.groupID in self.__shipPackagedVolumesPerGroup__ and item.typeID != const.typeBHMegaCargoShip:
            volume = self.__shipPackagedVolumesPerGroup__[item.groupID]
        elif item.typeID in self.__containerPackagedVolumesPerType__ and not item.singleton:
            volume = self.__containerPackagedVolumesPerType__[item.typeID]
        else:
    ...
    def GotInitData(self, initdata):
        ...
        if boot.GetValue('useFSD', 1) == 1:
            typeFile = 'res:/staticdata/typeIDs.static'
            typeSchema = 'res:/staticdata/typeIDs.schema'
            self.LogNotice('Loading type overrides from FSD')
            self.fsdTypeOverrides = self.LoadFSDData(typeFile, typeSchema, loadFromString=True)
            if blue.pyos.packaged is False:
                self.LogNotice('Loading test type overrides from FSD')
                testTypeFile = 'res:/staticdata/testTypeIDs.static'
                self.fsdTestTypeOverrides = self.LoadFSDData(testTypeFile, typeSchema, loadFromString=True)
            if boot.role == 'client':
                planetAttributesFile = 'res:/staticdata/planetAttributes.static'
                planetAttributesSchema = 'res:/staticdata/planetAttributes.schema'
                self.LogNotice('Loading Planet Attributes from FSD')
                self.fsdPlanetAttributes = self.LoadFSDData(planetAttributesFile, planetAttributesSchema, idCacheCount=100)
        ...
        if boot.role == 'client':
            ...
            self._averageMarketPrice = self.GetConfigSvc().GetAverageMarketPricesForServer()
        ...
        self.__shipPackagedVolumesPerGroup__ = {const.groupAssaultShip: 2500.0,
        ...
        self.__containerPackagedVolumesPerType__ = {const.typeGiantSecureContainer: 300,
    ...
    def GetAveragePricesThread(self):
        ...
        self._averageMarketPrice = self.GetConfigSvc().GetAverageMarketPricesForClient()
...
class InvType(Row):
    ...
    def IconID(self):
        iconID = Row.__getattr__(self, 'iconID')
        if cfg.fsdTypeOverrides is not None:
            try:
    ...
    def Radius(self):
        radius = Row.__getattr__(self, 'radius')
        if cfg.fsdTypeOverrides is not None:
            try:
    ...
    def SoundID(self):
        soundID = Row.__getattr__(self, 'soundID')
        if cfg.fsdTypeOverrides is not None:
            try:
...
BULKDEFINITIONS = {'npcStations': {'source': 'station.npcStationsVx',
 ...
 'celestials': {'source': 'map.celestialsVx',
                'keys': [('celestialID', 'integer PRIMARY KEY'),
                         ...
                         ('celestialIndex', 'integer')],
                'indexes': [['solarSystemID']]},
 'typeSkillReqs': {'source': 'inventory.typesBySkillLevelVx',
...
exports = {'util.GraphicFile': GraphicFile,
 ...
 'util.GetCharacterType': GetCharacterType,
 'util.IsOutlawStatus': IsOutlawStatus,
 'util.IsEvePlayerCharacter': IsEvePlayerCharacter}