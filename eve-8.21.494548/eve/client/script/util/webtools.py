...
class WebTools(service.Service):
    ...
    def DoRenderTypes(self, sz, blueprintCopy = False):
        ...
        print 'Rendering out %s %s' % (len(toRender), 'types' if not blueprintCopy else 'BPCs')
        numSkipped = 0
        if blueprintCopy:
        ...
        for counter, typeID in enumerate(toRender):
            try:
                ...
                if counter % 100 == 0:
                    msg = 'Rendering %sx%s icons: %s / %s' % (sz,
                     sz,
                     counter,
                     len(toRender))
                    print '\n', msg
    ...
    def IconsRender(self):
        ...
        Msg('All icons have been rendered. Generating report...')
        report = self.GenerateReport()
        Msg('Image Export Collection done. Report available here: %s' % report)
...
class IconRoutines():
    ...
    def BatchRender(self, toRender, folder, save = 1, iconsize = 64):
        ...
        notRender = []
        for i, invtype in enumerate(manyTypes):
            try:
                lr.WriteLogFile(' - rendering typeID %s: %s' % (invtype.typeID, invtype.typeName))
                Msg('BatchRender %s/%s: %s' % (i, len(manyTypes), invtype.typeName))
            except:
                Msg('BatchRender %s/%s: typeID %s' % (i, len(manyTypes), invtype.typeID))
    ...
    def TotalBatchRender(self, icons = None, categories = None, corporation = None):
        ...
        if categories:
            ...
            Msg('Rendering Tech3 ships...')
            self.RenderTech3Ships()
            try:
                Msg('Rendering Apparel...')
                self.RenderApparel()
            except Exception as e:
