...
class DecalBaker(object):
    ...
    def CreateTargetsOnModifier(self, modifier, asRenderTargets = False):
        ...
        if modifier.decalData.bodyEnabled:
            modifier.mapD[PD.DOLL_PARTS.BODY] = trinity.TriTextureRes(trinity.Tr2RenderTarget(self.size[0] / 2, self.size[1], 1, textureFormat))
        if modifier.decalData.headEnabled:
            modifier.mapD[PD.DOLL_PARTS.HEAD] = trinity.TriTextureRes(trinity.Tr2RenderTarget(self.size[0] / 2, self.size[1] / 2, 1, textureFormat))
