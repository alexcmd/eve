#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/login/charcreation/charCreation.py
import uiconst
import uicls
import random
import uthread
...
class RaceStep(BaseCharacterCreationStep):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.sr.raceCont = uicls.Container(name='raceCont', parent=self.sr.raceInfoCont, align=uiconst.TOTOP, pos=(0, 40, 0, 80), state=uiconst.UI_HIDDEN)
        self.raceSprite = uicls.Sprite(name='raceSprite', parent=self.sr.raceCont, align=uiconst.CENTER, state=uiconst.UI_DISABLED, texturePath=self.raceHeaderPath, pos=(0, 0, 512, 128))
        ...
        for race in sm.GetService('cc').GetRaceData(shuffleFirstTime=True):
            raceBtn = uicls.RaceButton(name='raceBtn', parent=self.sr.buttonCont, align=uiconst.TOLEFT, pos=(0, 0, 128, 128), raceID=race.raceID, mouseExitCallback=self.ResetHoverRaceID)
    ...
    def PlayMovie(self):
        ...
        if info.raceID is None:
            return
        sm.StartService('audio').StopLocationMusic('music_character_creation')
    ...
    def Pause(self, *args):
        if not uicore.layer.charactercreation.raceMusicStarted:
            sm.StartService('audio').PlayLocationMusic('music_character_creation')
        self.movie.Pause()
...
class CharacterBloodlineSelection(BaseCharacterCreationStep):
    ...
    def ApplyAttributes(self, attributes):
        ...
        for bloodlineID in self.bloodlineIDs:
            cont = uicls.Container(name='cont', parent=self.sr.uiContainer, align=uiconst.TOPLEFT, pos=(0, 0, 300, 200), state=uiconst.UI_HIDDEN)
            contName = 'cont_%d' % bloodlineID
            setattr(self.sr, contName, cont)
            contGender = uicls.Container(name='contGender', parent=cont, align=uiconst.CENTERBOTTOM, pos=(0, 4, 255, 64))
            ...
            btn = uicls.BloodlineButton(name='BloodlineButton', parent=cont, align=uiconst.CENTER, pos=(0, 0, 128, 128), bloodlineID=bloodlineID, mouseExitCallback=self.ResetHoverRaceID, mouseEnterCallback=self.HiliteCorrectBloodlines)
...
class CharacterPortrait(BaseCharacterCreationStep):
    ...
    __notifyevents__ += ['OnGraphicSettingsChanged', 'OnNCCgreenscreenChanged']
    ...
    def OnNCCgreenscreenChanged(self, *args):
        self.UpdateLayout()
...
class CharCreationButton(uicls.ButtonCore):
    ...
    def OnMouseExit(self, *args):
        uicls.ButtonCore.OnMouseExit(self, *args)
        if self.sr and self.sr.activeframe:
            self.sr.activeframe.display = False

    def OnMouseDown(self, *args):
        uicls.ButtonCore.OnMouseDown(self, *args)
        if self.sr and self.sr.activeframe:
...
class BitSlider(uicls.Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self._value = 0.0
        self.bitParent = uicls.Container(parent=self, pos=(0,
         0,
         targetWidth,
         self.height), align=uiconst.TOPLEFT)
        self.sr.handle = uicls.Container(parent=self.bitParent, align=uiconst.RELATIVE, state=uiconst.UI_DISABLED, pos=(0,
         0,
         3,
        ...
        while True:
            if bitAmount is None and i >= 3 and i * (self.bitWidth + bitGap) + self.bitWidth > targetWidth:
                break
            bit = uicls.Container(parent=self.bitParent, pos=(i * (self.bitWidth + bitGap),
             2,
             self.bitWidth,
...
class GradientSlider(uicls.Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.gradientSprite = uicls.GradientSprite(parent=self, pos=(0,
         0,
         targetWidth,
