#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/login/charcreation/assetRenderer.py
import service
import uicls
import uiconst
import blue
            ...
            if each.name == 'cameraUpdate':
                ...
                for child in each.steps:
                    if isinstance(child, trinity.TriStepClear):
                        child.enabled = False
                    elif hasattr(child, 'name') and child.name == 'RENDER_BACKDROP':
                        child.enabled = False
