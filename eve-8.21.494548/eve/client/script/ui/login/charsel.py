#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/login/charsel.py
import trinity
import uix
import uiutil
import blue
...
if boot.region != 'optic':
    ACCOUNT_MGMT_URL = 'https://secure.eveonline.com/login.asp?username=%(userName)s'
else:
    ACCOUNT_MGMT_URL = 'http://pay.tiancity.com/eve/EveExchangeMain.aspx'
...
class CharSelection(uicls.LayerCore):
    ...
    def InitUI(self):
        ...
        if session.userType == const.userTypeTrial:
            self.sr.trialCont = uicls.Container(name='trialCont', parent=charCont, align=uiconst.TOTOP, height=48, padBottom=16)
    ...
    def SetupTrialText(self):
        ...
        uicls.EveCaptionMedium(text=localization.GetByLabel('UI/CharacterSelection/DaysRemainingOfTrial'), parent=self.sr.trialCont, align=uiconst.CENTERLEFT, left=50, color=(1, 1, 1, 1), state=uiconst.UI_NORMAL)
        trialButtonContainer = uicls.Container(name='trialButtonContainer', parent=self.sr.trialCont, align=uiconst.CENTERRIGHT, left=9, height=30, color=(1, 1, 1, 1))
        trialButtonCenterContainer = uicls.Container(name='trialButtonCenterContainer', parent=trialButtonContainer, align=uiconst.TOTOP, height=30, state=uiconst.UI_NORMAL)
        buttonText = uicls.Label(text=localization.GetByLabel('UI/CharacterSelection/SubscribeToEve'), parent=trialButtonCenterContainer, align=uiconst.CENTER, fontsize=16, idx=0)
        buttonWidth = buttonText.width + 12
        trialButtonContainer.width = buttonWidth
        btn = xtriui.BigButton(parent=trialButtonCenterContainer, left=0, top=0, width=buttonWidth, height=30, idx=1)
        btn.Startup(buttonWidth, 30)
        btn.OnClick = self.OnTrialSubscribeButtonPressed
        btn.sr.hilite.padding = 3
        uicls.Fill(parent=btn, color=(0.1, 0.1, 0.1, 1))
        uicls.Fill(parent=self.sr.trialCont, color=(1, 1, 1, 0.15))
    ...
    def OnTrialSubscribeButtonPressed(self):
        uicore.cmd.OpenBrowser(ACCOUNT_MGMT_URL % {'userName': settings.public.ui.Get('username', '')})
    ...
    def SetupSlots(self, reload = False):
        ...
        self.sr.infoButtonCont = uicls.Container(name='infoButtonCont', parent=self.sr.subparent, align=uiconst.TOPLEFT, pos=(4, 0, 100, 24), idx=0)
    ...
    def SetupButtons(self):
        ...
        self.sr.tipofthedayText = uicls.Label(text=currentText, parent=self.sr.totdCont, align=uiconst.TOALL, fontsize=self.fontSize + 3, top=self.sr.tipofthedayCaption.top + self.sr.tipofthedayCaption.textheight, left=20, color=None, state=uiconst.UI_NORMAL, idx=0)
    ...
    def AddCharacter(self, char, slotidx):
        ...
        if slotidx == 0:
            ...
            slot.sr.textcont = uicls.Container(parent=self.sr.infoButtonCont, name='textCont', align=uiconst.TOPLEFT, state=uiconst.UI_DISABLED, pos=(32, 5, 70, 50))
    ...
    def InitScene(self):
        sm.GetService('sceneManager').LoadScene('res:/dx9/Scene/CharselectScene2.red', 'charsel', leaveUntouched=0, setupCamera=False)
    ...
    def ReduceCharacterGraphics(self):
        ...
        settings.public.device.Set('charClothSim', 0)
