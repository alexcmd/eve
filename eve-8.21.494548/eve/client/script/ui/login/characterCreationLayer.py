#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/login/characterCreationLayer.py
import uicls
import uiutil
import uiconst
import trinity
...
class CharacterCreationLayer(uicls.LayerCore):
    ...
    def OnOpenView(self):
        ...
        if previewWnd and previewWnd.previewingWhat == 'character':
            self.previewWindowWasOpenOn = previewWnd.previewCharacterInfo
        ...
        self.fastCharacterCreation = settings.public.device.Get('fastCharacterCreation', 0)
    ...
    def Save(self, *args):
        try:
            if self.GetBackgroundID() > const.NCC_MAX_NORMAL_BACKGROUND_ID:
            ...
            if self.charID and self.mode in [ccConst.MODE_FULL_RECUSTOMIZATION, ccConst.MODE_LIMITED_RECUSTOMIZATION, ccConst.MODE_FULL_BLOODLINECHANGE]:
    ...
    def IsSlowMachine(self):
        if settings.public.device.Get('fastCharacterCreation', False):
            return True
        if settings.public.device.Get('shaderQuality', 3) < 2:
            return True
        return False
    ...
    def OnNCCgreenscreenChanged(self, *args):
        self.UpdateBackdrop()
        if self.floor:
            self.floor.display = not settings.user.ui.Get('NCCgreenscreen', 0)
    ...
    def UpdateBackdrop(self, *args):
        ...
        if self.stepID == ccConst.RACESTEP:
            ...
            if settings.user.ui.Get('NCCgreenscreen', 0):
                bgSize = max(desktopWidth, desktopHeight)
                backdropSprite = trinity.Tr2Sprite2d()
    ...
    def ExitToStation(self, updateDoll = True):
        if session.worldspaceid is not None and settings.public.device.Get('loadstationenv2', 1):
            dna = self.GetDNA()
    ...
    def ReApplyClothes(self, character, doUpdate = True):
        if self.clothesDNAStorage is None or character is None:
            return
        character.doll.MatchDNA(character.factory, self.clothesDNAStorage)
        self.ResetClothesStorage()
        if doUpdate:
    ...
    def RemoveClothes(self, character, doUpdate = True):
        ...
        self.ResetClothesStorage()
        self.clothesDNAStorage = character.doll.GetDNA()
    ...
    def AnimateToStoredPortrait_thread(self, newPortraitID):
        ...
        if path in ccConst.backgroundOptions or settings.user.ui.Get('NCCgreenscreen', 0) and path in ccConst.greenscreenBackgroundOptions:
            self.backdropPath = path
        elif not settings.user.ui.Get('NCCgreenscreen', 0):
    ...
    def DoLogEvent(self, finished = 1, *args):
        ...
        openFor = round(float(endTime - self.startTime) / SEC)
...
class CCConfirmationWindow(uicls.Window):
    ...
    def ApplyAttributes(self, attributes):
        ...
        caption = uicls.EveCaptionMedium(text=localization.GetByLabel('UI/CharacterCreation/ConfirmationHeader'), parent=self.sr.rightSide, align=uiconst.TOTOP, idx=-1, state=uiconst.UI_DISABLED, name='caption')
        text = uicls.EveLabelMedium(text=localization.GetByLabel('UI/CharacterCreation/Confirmation'), parent=self.sr.rightSide, align=uiconst.TOTOP, top=4)
