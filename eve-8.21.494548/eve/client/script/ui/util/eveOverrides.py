#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/util/eveOverrides.py
import uiutil
import blue
import log
import uicls
...
def PrepareDrag_Override(dragContainer, dragSource, *args):
    ...
    for node in dragContainer.dragData:
        ...
        if decoClass is not None and issubclass(decoClass, xtriui.InvItem) or node.__guid__ in ('xtriui.FittingSlot', 'xtriui.ModuleButton', 'xtriui.ShipUIModule'):
            isCopy = False
        ...
        if node.__guid__ in ('xtriui.TypeIcon', 'listentry.DraggableItem', 'uicls.GenericDraggableForTypeID'):
        ...
        elif node.__guid__ in ('listentry.QuickbarItem', 'listentry.GenericMarketItem', 'listentry.DirectionalScanReults'):
            typeID = node.typeID
        ...
        elif node.__guid__ in ('listentry.KillMail', 'listentry.KillMailCondensed', 'listentry.WarKillEntry'):
            icon.LoadIcon('ui_1337_64_1')
        elif node.__guid__ in 'listentry.WarEntry':
            icon.LoadIcon('ui_1337_64_2')
        elif node.__guid__ in 'listentry.TutorialEntry':
            icon.LoadIcon('74_13')
        elif node.__guid__ in ('xtriui.CertSlot', 'listentry.CertEntry'):
            icon.LoadIcon('79_1')
        elif isinstance(node, (uiutil.TreeDataInv, uiutil.TreeDataInvFolder)):
            icon.LoadIcon(node.GetIcon())
        elif node.__guid__ in 'listentry.RecruitmentEntry':
            corpID = node.corporationID
            uix.GetOwnerLogo(icon, corpID, iconSize, noServerCall=False)
        elif node.__guid__ == 'uiutil.InfoPanelDragData':
...
uiutil.PrepareDrag_Override = PrepareDrag_Override