#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/util/uix.py
import sys
import types
import log
import util
...
def GetItemName(invItem, data = None):
    ...
    elif invItem.singleton and (invItem.categoryID == const.categoryShip or invItem.groupID in (const.groupWreck,
...
def GetTextHeight(strng, width = 0, fontsize = fontConst.EVE_MEDIUM_FONTSIZE, linespace = None, hspace = 0, uppercase = 0, specialIndent = 0, getTextObj = 0, tabs = [], maxLines = None, **kwds):
    return sm.GetService('font').GetTextHeight(strng, width=width, font=None, fontsize=fontsize, linespace=linespace, letterspace=hspace, uppercase=uppercase, specialIndent=specialIndent, getTextObj=getTextObj, tabs=tabs, maxLines=maxLines)
...
def __Search(searchStr, groupID, exact, filterCorpID, hideNPC = False):
    ...
    if groupID == const.groupCharacter:
        ...
        result = searchUtil.QuickSearch(searchStr, [const.searchResultSolarSystem], exact)
...
def ListWnd(lst, listtype = None, caption = None, hint = None, ordered = 0, minw = 200, minh = 256, minChoices = 1, maxChoices = 1, initChoices = [], validator = None, isModal = 1, scrollHeaders = [], iconMargin = 0, windowName = 'listwindow', lstDataIsGrouped = 0, unstackable = 0, noContentHint = None):
    ...
    wnd = form.ListWindow(windowID=windowName, lst=lst, listtype=listtype, ordered=ordered, minSize=(minw, minh), caption=caption, minChoices=minChoices, maxChoices=maxChoices, initChoices=initChoices, validator=validator, scrollHeaders=scrollHeaders, iconMargin=iconMargin, lstDataIsGrouped=lstDataIsGrouped, noContentHint=noContentHint)
    if hint:
...
def SetStateFlagForFlag(container, flag, charID = None, top = 20, left = 4, showHint = True):
    if flag and flag != -1:
        ...
        if filterName:
            ...
            flagIcon = container.sr.flag.children[0]
            flagBackground = container.sr.flag.children[1]
            uiSvc = sm.GetService('ui')
            flagIcon.color.SetRGBA(*props.iconColor)
            col = stateSvc.GetStateFlagColor(flag)
            blink = stateSvc.GetStateFlagBlink(flag)
            flagBackground.color.SetRGB(*col)
            flagBackground.color.a *= 0.75
            if blink:
                uiSvc.BlinkSpriteA(flagIcon, 1.0, 500, None, passColor=0)
                uiSvc.BlinkSpriteA(flagBackground, flagIcon.color.a, 500, None, passColor=0)
            else:
                uiSvc.StopBlink(flagIcon)
                uiSvc.StopBlink(flagBackground)
