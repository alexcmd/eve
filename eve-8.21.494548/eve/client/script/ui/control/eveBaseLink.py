#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/control/eveBaseLink.py
import uicls
import util
import log
import blue
...
HINTLABELS = {'showinfo': 'UI/Commands/ShowInfo',
 ...
 'CertSlot': 'UI/Certificates/PlannerWindow/OpenCertificationPlanner',
...
class BaseLink(uicls.BaseLinkCore):
    ...
    def ClickGameLinks(self, parent, URL):
        ...
        if URL.startswith('CertEntry:'):
            myArgs = URL[10:]
            certID, grade = myArgs.split('//')
            abstractinfo = util.KeyVal(certificateID=int(certID), grade=int(grade))
        ...
        if URL.startswith('killReport:'):
            ...
            if kill is not None:
                form.KillReportWnd.CloseIfOpen()
        ...
        if URL.startswith('tutorial:'):
            ...
            sm.GetService('tutorial').OpenTutorialFromOutside(int(tutorialID), ask=True)
            return True
        ...
        if URL.startswith('joinChannel:'):
            ...
            if channelID and not sm.GetService('LSC').IsJoined(channelID):
                ...
                if adID is not None:
                    sm.GetService('corp').LogCorpRecruitmentEvent('TalkToRecruiter', session.corpid, session.allianceid, channelID, corpID, adID)
            return True
        return False
    ...
    def GetLinkMenu(self, parent, url):
        ...
        if url.startswith('showinfo:'):
            ...
            if url.startswith('preview:'):
                return []
            if url.startswith('contract:'):
                m += [(uiutil.MenuLabel('UI/Contracts/ShowContract'), self.Contract, (url[9:],))]
                return m
            if url.startswith('CertSlot:'):
                m += [(uiutil.MenuLabel('UI/Commands/OpenCertificatePlanner'), sm.StartService('certificates').OpenCertificateWindow, (url[9:],))]
                return m
        if self.ValidateURL(url):
            url = url.replace('&amp;', '&')
            url = browserutil.GetFixedURL(parent, url)
            m += [(uiutil.MenuLabel('UI/Browser/OpenLinkInNewTab'), self.UrlHandlerDelegate, (parent,
               'NewView',
               url,
               True))]
            m += [(uiutil.MenuLabel('UI/Common/Open'), self.UrlHandlerDelegate, (parent,
               'GoTo',
               url,
               False))]
