#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/services/eveCalendarsvc.py
import uiconst
import service
import svc
import blue
...
class EveCalendarSvc(svc.calendar):
    ...
    def LoadCCPIcon(self, container, top, left, *args):
        if getattr(container.sr, 'flag', None) is None:
            container.sr.flag = uicls.Container(parent=container, pos=(0, 0, 11, 11), name='flag', state=uiconst.UI_DISABLED, align=uiconst.TOPRIGHT)
            uicls.Sprite(parent=container.sr.flag, pos=(0, 0, 11, 11), name='icon', state=uiconst.UI_DISABLED, rectWidth=10, rectHeight=10, texturePath='res:/UI/Texture/classes/Bracket/flagIcons.png', align=uiconst.RELATIVE)
    ...
    def LoadAutomatedIcon(self, container, top, left, *args):
        if getattr(container.sr, 'flag', None) is None:
            container.sr.flag = uicls.Container(parent=container, pos=(0, 0, 11, 11), name='flag', state=uiconst.UI_DISABLED, align=uiconst.TOPRIGHT, left=left, top=top - 1)
            uicls.Sprite(parent=container.sr.flag, pos=(0, 0, 9, 9), left=1, top=1, name='icon', state=uiconst.UI_DISABLED, texturePath='res:/UI/Texture/Icons/AutomatedEntry_Icon_Alpha.png')
            col = sm.GetService('state').GetStateFlagColor(state.flagSameCorp)
            fill = uicls.Fill(parent=container.sr.flag)
        container.sr.flag.children[1].color.SetRGB(*col)
