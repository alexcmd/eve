#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/services/sovSvc.py
import blue
import service
import form
import util
...
class SovService(service.Service):
    ...
    def GetSystemSovereigntyInfo(self, solarSystemID, forceUpdate = False):
        if self.facwar.IsFacWarSystem(solarSystemID):
            return util.KeyVal(allianceID=self.facwar.GetSystemOccupier(solarSystemID), corporationID=None, claimStructureID=None, hubID=None, claimTime=None, contested=0)
    ...
    def AddUppgrades(self, hubID, itemlist, sourceLocationID):
        ...
        inv.MultiAdd(itemlist, sourceLocationID, flag=const.flagNone)
    ...
    def GetDominantAllianceID(self, scope, reqRegionID, reqConstellationID, reqSolarSystemID):
        ...
        fwdata = self.facwar.GetFacWarSystemsOccupiers()
        for info in data:
            systemOwners[info.solarSystemID] = info.allianceID

        for systemID, occupierID in fwdata.iteritems():
            systemOwners[systemID] = occupierID
