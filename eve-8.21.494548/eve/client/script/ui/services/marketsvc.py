#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/services/marketsvc.py
from service import *
import util
import blue
import dbutil
...
class MarketQuote(Service):
    ...
    def DumpQuotes(self, typeID, amount = 1):
        ...
        for bidRange, name in ranges.iteritems():
            ...
            if ask is None:
                ...
                print '- In %15s' % name, ': Seller %s jumps away, selling %s units at %s ISK at station %s %s wide' % (int(sm.GetService('pathfinder').GetJumpCountFromCurrent(ask.solarSystemID)),
                 ask.volRemaining,
                 ask.price,
                 ask.stationID,
                 ranges[ask.range])
    ...
    def DumpOrdersForType(self, typeID):
        ...
        typeName = cfg.invtypes.Get(typeID).name
        fileTypeName = uiutil.StripTags(typeName).replace(localization.HIGHLIGHT_IMPORTANT_MARKER, '')
        fileTypeName = fileTypeName.replace('"', '')
        fileRegionName = uiutil.StripTags(cfg.evelocations.Get(session.regionid).name).replace(localization.HIGHLIGHT_IMPORTANT_MARKER, '')
        directory = blue.win32.SHGetFolderPath(blue.win32.CSIDL_PERSONAL) + '\\EVE\\logs\\Marketlogs\\'
        filename = '%s-%s-%s.txt' % (fileRegionName, fileTypeName.replace('/', '_'), util.FmtDateEng(blue.os.GetWallclockTime()).replace(':', ''))
