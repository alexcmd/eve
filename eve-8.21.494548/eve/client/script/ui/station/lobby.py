...
class Lobby(uicls.Window):
    ...
    def LayoutShipsAndItems(self):
        ...
        invCont.StationItems(name='stationItems', parent=self.sr.itemsContainer, showControls=True, padding=const.defaultPadding)
        self.sr.lobbytabs.ShowPanel(self.sr.itemsContainer)
        self.sr.shipsContainer.Flush()
        invCont.StationShips(name='stationShips', parent=self.sr.shipsContainer, showControls=True, padding=const.defaultPadding)
    ...
    def OnSvcBtnEnter(self, btn, *args):
        eve.Message('%sBtnEnter' % btn.name)
    ...
    def ReleaseImpoundedItems(self, *args):
        ...
        if eve.Message('CrpJunkAcceptCost', {'cost': util.FmtAmt(cost)}, uiconst.YESNO) != uiconst.ID_YES:
            return
    ...
    def LoadOwnerInfo(self):
        ...
        if not uiutil.CheckCorpID(corpID):
            self.sr.corpCaption = caption = uicls.CaptionLabel(text='<center>' + cfg.eveowners.Get(corpID).name, parent=parent, align=uiconst.TOTOP, idx=0, name='corpname', padTop=logo.top + logo.height + 4, uppercase=False)
...
class OfficeEntry(uicls.SE_BaseClassCore):
    ...
    def Startup(self, *args):
        ...
        fieldName = 'corpName'
        l = uicls.EveLabelSmall(text=label, parent=par, left=5, top=2, state=uiconst.UI_DISABLED)
