#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/station/fitting/base_fitting.py
import sys
import uix
import uiutil
import mathUtil
...
class Fitting(uicls.FittingLayout):
    ...
    def _Anim(self):
        ...
        self.sr.shipnametext = uicls.EveLabelMedium(text='', parent=self.sr.shipnamecont, width=250, state=uiconst.UI_DISABLED)
        ...
        em = xtriui.ExpandableMenuContainer(parent=rightside, pos=(0, 0, 0, 0), clipChildren=True)
        ...
        for i in xrange(8):
            cos = math.cos(angle * math.pi / 180.0)
            sin = math.sin(angle * math.pi / 180.0)
            iconPar = uicls.Transform(parent=newslotParent, pos=(0,
             0,
             16,
             rad * 2 + 16), align=uiconst.CENTER)
            iconPar.SetRotation(-mathUtil.DegToRad(angle + 90.0))
            icon = uicls.Sprite(texturePath='res:/UI/Texture/classes/Fitting/slotLeft.png', parent=iconPar, width=16, height=16)
            icon.hint = launcherHardpointsName
            cos = math.cos((180.0 - angle) * math.pi / 180.0)
            sin = math.sin((180.0 - angle) * math.pi / 180.0)
            iconPar2 = uicls.Transform(parent=newslotParent, pos=(0,
             0,
             16,
             rad * 2 + 16), align=uiconst.CENTER)
            iconPar2.SetRotation(mathUtil.DegToRad(angle + 90.0))
            icon2 = uicls.Sprite(texturePath='res:/UI/Texture/classes/Fitting/slotLeft.png', parent=iconPar2, width=16, height=16)
            icon2.hint = turretHardpointName
    ...
    def SaveFittingSetup(self, *args):
        ...
        form.ViewFitting.Open(windowID=windowID, fitting=fitting, truncated=None)
    ...
    def LoadNavigationStats(self, initialLoad = False):
        ...
        for attrGroup in toShow:
            row = uicls.Container(name='topRow', parent=np, align=uiconst.TOTOP, height=26, padTop=3)
            left = 3
    ...
    def LoadDefenceStats(self, initialLoad = False):
        ...
        statusLabel = uicls.Label(text='', parent=bestPar, left=icon.left + icon.width, state=uiconst.UI_DISABLED, align=uiconst.CENTERLEFT)
        ...
        for label, what, iconNo, labelhint, iconhint in ((localization.GetByLabel('UI/Common/Shield'),
          ...
          '')):
            ...
            statusLabel = uicls.Label(text='', parent=row, left=mainIcon.left + mainIcon.width, state=uiconst.UI_NORMAL, align=uiconst.CENTERLEFT, width=62)
    ...
    def _UpdateStats(self):
        ...
        for slotSet, slotsLeft, slotsFitted, slotsAddition in [(self.sr.turretSlots,
          turretSlotsLeft,
          turretsFitted,
          turretAddition), (self.sr.launcherSlots,
          launcherSlotsLeft,
          launchersFitted,
          launcherAddition)]:
    ...
    def ExpandBestRepair(self, *args):
        ...
        for flag, hint, iconNo in ((ARMORREPAIRRATEACTIVE, localization.GetByLabel('UI/Fitting/FittingWindow/ArmorRepairRate'), 'ui_1_64_11'),
         ...
         (SHIELDBOOSTRATEACTIVE, localization.GetByLabel('UI/Fitting/FittingWindow/ShieldBoostRate'), 'ui_2_64_3')):
            ...
            label = uicls.Label(text=hint, parent=entry, left=icon.left + icon.width, state=uiconst.UI_DISABLED, align=uiconst.CENTERLEFT)
