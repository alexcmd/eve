...
class HangarLayer(uicls.LayerCore):
    ...
    def OnDropData(self, dragObj, nodes):
        ...
        if util.LocalSvcHasAttr(station, 'OnStuffDropOnMainArea'):
            uthread.new(station.OnStuffDropOnMainArea, nodes)
    ...
    def OnMouseWheel(self, *args):
        if not self.staticEnv:
            ...
            if self.camera.__typename__ == 'EveCamera':
                self.camera.Dolly(uicore.uilib.dz * 0.001 * abs(self.camera.translationFromParent))
