#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/station/lpstore.py
import blue
import const
import form
import listentry
...
class LPOfferEntry(uicls.SE_BaseClassCore):
    ...
    def ApplyAttributes(self, attributes):
        ...
        subPar = uicls.Container(parent=self.sr.rewardParent, name='rewardLabelClipper', state=uiconst.UI_PICKCHILDREN, align=uiconst.TOALL, clipChildren=True)
        ...
        self.sr.buyBtn = uicls.UtilMenu(name='buyBtn', parent=subPar, label=localization.GetByLabel('UI/VirtualGoodsStore/Buttons/Buy'), align=uiconst.CENTER, GetUtilMenu=self.GetBuyUtilMenu, texturePath=None)
        self.sr.buyBtn.icon.Hide()
        self.sr.buyBtn._label.align = uiconst.CENTER
        self.sr.buyBtn._label.left = 0
        uicls.Frame(bgParent=self.sr.buyBtn, name='dot', texturePath='res:/UI/Texture/Shared/windowButtonDOT.png', cornerSize=6, spriteEffect=trinity.TR2_SFX_DOT)
    ...
    def GetBuyUtilMenu(self, menuParent):
        return self._GetBuyUtilMenu(menuParent)

    def _GetBuyUtilMenu(self, menuParent):
        cont = uicls.ContainerAutoSize(parent=menuParent, align=uiconst.TOTOP, padding=const.defaultPadding)
        cont.GetEntryWidth = lambda mc = cont: 80
        self.amountEdit = uicls.SinglelineEdit(name='amountEdit', parent=cont, align=uiconst.TOTOP, ints=[1, const.maxLoyaltyStoreBulkOffers], setvalue=1, label=localization.GetByLabel('UI/Common/Amount'), padding=(5, 10, 5, 0), OnReturn=self.Buy)
        uicls.Button(name='buyButton', parent=cont, align=uiconst.TOTOP, label=localization.GetByLabel('UI/LPStore/Accept'), func=self.Buy, padding=(5, 5, 5, 5))
        uicore.registry.SetFocus(self.amountEdit)

    def Buy(self, *args):
        sm.GetService('lpstore').AcceptOffer(self.sr.node, self.amountEdit.GetValue())
...
class LPStoreWindow(uicls.Window):
    ...
    def ConstructHeader(self):
        ...
        self.sr.corpLabel = uicls.Label(parent=self.sr.topParent, name='corpLabel', align=uiconst.TOPRIGHT, fontsize=18, left=10, state=uiconst.UI_NORMAL)
        self.sr.corpLPsLabel = uicls.EveLabelMedium(parent=self.sr.topParent, name='corpLPsLabel', align=uiconst.TOPRIGHT, left=10, state=uiconst.UI_NORMAL)
        xchangeRate = sm.GetService('lpstore').GetConcordLPExchangeRate()
        self.sr.exchangeButton = uicls.Button(parent=self.sr.topParent, name='exchangeBtn', align=uiconst.TOPRIGHT, left=5, label=localization.GetByLabel('UI/LPStore/ConcordExchange'))
        filterParent = uicls.Container(parent=self.sr.topParent, name='filterParent', pos=(100, 0, 200, 55), align=uiconst.RELATIVE)
        self.sr.presetCombo = uicls.Combo(parent=filterParent, name='presetCombo', left=5, top=20, adjustWidth=True, label=localization.GetByLabel('UI/LPStore/OfferFilter'))
        self.sr.filterBtn = uicls.Button(parent=filterParent, name='filterBtn', left=5, top=self.sr.presetCombo.top + self.sr.presetCombo.height + const.defaultPadding, label=localization.GetByLabel('UI/LPStore/EditFilters'))

    def ConstructContent(self):
        self.sr.lpStoreScroll = uicls.Scroll(parent=self.sr.main, name='lpStoreScroll', align=uiconst.TOALL, padding=const.defaultPadding)
...
class LPStoreFiltersWindow(uicls.Window):
    ...
    def ConstructContent(self):
        ...
        c = uicls.Container(parent=tab, name='currentPresetHeaderParent', height=16, align=uiconst.TOTOP)
        LPStoreHeaderLabel(name='currentPresetHeader', parent=c, text=localization.GetByLabel('UI/LPStore/ActivePresetName'))
        c = uicls.Container(parent=tab, name='currentPresetLabelParent', height=16, align=uiconst.TOTOP)
...
class LPExhangeDialog(uicls.Window):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.amountEdit = uicls.SinglelineEdit(name='lpAmountEdit', parent=cont, pos=(17, 0, 80, 0), ints=(0, self.maxCorporationLPs), align=uiconst.TOPRIGHT)
