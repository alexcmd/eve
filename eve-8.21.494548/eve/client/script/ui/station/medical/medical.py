#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/station/medical/medical.py
import base
import blue
import form
import listentry
...
class MedicalWindow(uicls.Window):
    ...
    def ApplyAttributes(self, attributes):
        ...
        uicls.WndCaptionLabel(text=localization.GetByLabel('UI/Medical/Cloning'), subcaption=localization.GetByLabel('UI/Medical/Clone/CurrentSkillPoints', skillPoints=util.FmtAmt(currentSkillPoints)), parent=self.sr.topParent, align=uiconst.RELATIVE)
        self.scope = 'station'
        ...
        try:
            ...
            for typeInfo in cfg.typesByGroups.get(const.groupClone, []):
                ...
                if godmaInfo.skillPointsSaved <= currentCloneGodmaInfo.skillPointsSaved:
                    continue
                price = facwarCommon.GetAdjustedClonePrice(session.solarsystemid2, session.warfactionid, typeInfo.basePrice)
                cloneList.append([typeInfo.typeID, godmaInfo.skillPointsSaved, price])
            ...
            ret = uix.ListWnd(cloneDisplayList, 'type', localization.GetByLabel('UI/Medical/Clone/SelectCloneType'), hint, 0, 420, windowName='cloneUpgradesListWnd')
            if ret:
            ...
            price = facwarCommon.GetAdjustedClonePrice(session.solarsystemid2, session.warfactionid, cfg.invtypes.Get(newCloneTypeID).basePrice)
            if eve.Message('AskAcceptCloneTypeCost', {'cost': price}, uiconst.YESNO) != uiconst.ID_YES:
                self.picking = 0
                return
