    ...
    def OnMouseUp(self, button, *args):
        ...
        if not uicore.uilib.rightbtn:
            ...
            if camera.__typename__ == 'EveCamera':
                if not sm.GetService('camera').tracking:
    ...
    def OnMouseMove(self, *args):
        ...
        if (self.looking or self.zoomlooking) and camera.__typename__ == 'EveCamera':
            ...
            if alt and cameraSvc.IsFreeLook():
                ...
                if rightBtn:
                    camera.Dolly(-0.01 * dy * abs(camera.translationFromParent))
                    camera.translationFromParent = cameraSvc.CheckTranslationFromParent(camera.translationFromParent)
                if midBtn:
