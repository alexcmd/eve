#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/inflight/bracketsAndTargets/inSpaceBracket.py
import weakref
import blue
import telemetry
import util
...
class InSpaceBracket(uicls.Bracket):
    ...
    def ShowLabel(self, *args):
        if not self.destroyed and (self.displayName == '' or not getattr(self, 'showLabel', True)):
            return
        if not self.label:
            self.label = uicls.BracketLabel(parent=self.parent, name='labelparent', idx=0, align=uiconst.TOPLEFT, state=uiconst.UI_NORMAL, text=self.displayName, bracket=self)
        if not self.subLabel and self.displaySubLabel:
            self.subLabel = uicls.BracketLabel(parent=self.parent, name='sublabelparent', align=uiconst.TOPLEFT, state=uiconst.UI_NORMAL, text=self.displaySubLabel, bracket=self)
    ...
    def GetHitSprite(self, create = True, *args):
        ...
        hitSprite = uicls.Sprite(parent=self, name='hitSprite', pos=(0, 0, 100, 100), state=uiconst.UI_PICKCHILDREN, texturePath=texturePath, align=uiconst.CENTER)
    ...
    def GetCanTargetSprite(self, create = True, *args):
        ...
        if not create:
            return
        canTargetSprite = uicls.Sprite(parent=self, name='canTargetSprite', pos=(0, 0, 40, 40), state=uiconst.UI_PICKCHILDREN, texturePath='res:/UI/Texture/classes/Bracket/canTarget2.png', align=uiconst.CENTER)
    ...
    def AddOwnShipIcon(self, *args):
        ...
        self.ownShipIcon = uicls.Icon(parent=self, pos=(0, 0, 16, 16), name='ownShipIcon', state=uiconst.UI_DISABLED, icon=props[0], align=uiconst.CENTER, color=(1.0, 1.0, 1.0, 0.75), glowExpand=0.1, glowColor=(0, 1.0, 1.0, 0.1))
            ...
            if slimItem.groupID != const.groupAgentsinSpace and (slimItem.ownerID and util.IsNPC(slimItem.ownerID) or slimItem.charID and util.IsNPC(slimItem.charID)):
                ...
                if backgroundFlag and backgroundFlag != -1:
                    ...
                    if not self.sr.bgColor:
                        self.sr.bgColor = uicls.Sprite(bgParent=self, name='bgColor', texturePath='res:/UI/Texture/classes/Bracket/bracketBackground.png', color=(r,
                         g,
                         b,
                         a))
                    else:
    ...
    def UpdateFleetTagAndTarget(self):
        ...
        if tagAndTargetStr:
            if not self.fleetTagAndTarget:
                self.fleetTagAndTarget = uicls.BracketLabel(parent=self.parent, name='fleetTagAndTarget', align=uiconst.TOPLEFT, state=uiconst.UI_NORMAL, text=tagAndTargetStr, fontsize=fontConst.EVE_MEDIUM_FONTSIZE, bracket=self, bold=True, idx=0)
            else:
    ...
    def FleetBroadcast(self, active, broadcastType, fleetBroadcastID, charID):
        if active:
            ...
            if not self.sr.icon and self.stateItemID != eve.session.shipid:
                self.LoadIcon(self.iconNo)
            self.fleetBroadcastIcon = uicls.BracketSubIconNew(icon=icon, parent=self.parent, state=uiconst.UI_NORMAL, name='fleetBroadcastIcon', bracket=self, hint=fleetbr.GetBroadcastName(broadcastType), idx=0)
            self.UpdateSubItems()
