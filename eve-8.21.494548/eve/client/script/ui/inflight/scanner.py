#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/inflight/scanner.py
import uix
import uiutil
import uiconst
import mathUtil
...
class Scanner(uicls.Window):
    ...
    def ApplyAttributes(self, attributes):
        ...
        btn = uix.GetBigButton(32, topParent, left=48)
        ...
        btn = uix.GetBigButton(32, topParent, left=84)
        ...
        btn = uix.GetBigButton(32, topParent, left=120)
        ...
        directionSettingsBox = uicls.Container(name='direction', parent=directionBox, align=uiconst.TOTOP, height=75, clipChildren=True)
        useOverviewLabel = localization.GetByLabel('UI/Inflight/Scanner/UseActiveOverviewSettings')
        useTrackingLabel = localization.GetByLabel('UI/Inflight/Scanner/Tracking')
        self.sr.useoverview = uicls.Checkbox(text=useOverviewLabel, parent=directionSettingsBox, configName='', retval=0, checked=settings.user.ui.Get('scannerusesoverviewsettings', 0), left=5, align=uiconst.TOPLEFT, callback=self.UseOverviewChanged, width=320, wrapLabel=False)
        self.sr.usetracking = uicls.Checkbox(text=useTrackingLabel, parent=directionSettingsBox, configName='', retval=0, checked=settings.user.ui.Get('scannerusestrackingcamera', 0), left=5, align=uiconst.TOPLEFT, top=self.sr.useoverview.top + 15, callback=self.UseTrackingChanged, width=300)
        self.dir_rangeinput = uicls.SinglelineEdit(name='edit', parent=directionSettingsBox, ints=(1, None), align=uiconst.TOPLEFT, pos=(5, 52, 85, 0), maxLength=len(str(sys.maxint)) + 1)
        self.dir_rangeinput.SetValue(settings.user.ui.Get('dir_scanrange', 1000))
        self.dir_rangeinput.OnReturn = self.DirectionSearch
        uicls.EveLabelSmall(text=localization.GetByLabel('UI/Inflight/Scanner/RangeKm'), parent=directionSettingsBox, width=240, left=6, top=38, state=uiconst.UI_DISABLED)
        uicls.Button(parent=directionSettingsBox, label=localization.GetByLabel('UI/Inflight/Scanner/Scan'), pos=(180, 52, 0, 0), func=self.DirectionSearch)
        slider = uix.GetSlider('slider', directionSettingsBox, 'scanangle', 5, 360, localization.GetByLabel('UI/Inflight/Scanner/Angle'), '', uiconst.RELATIVE, 90, 18, 90, 52, getvaluefunc=self.GetSliderValue, endsliderfunc=self.EndSetSliderValue, gethintfunc=lambda idname, dname, value: localizationUtil.FormatNumeric(int(round(value))), increments=(5, 15, 30, 60, 90, 180, 360), underlay=0)
        lbl = uiutil.GetChild(slider, 'label')
        lbl.top = -13
        lbl.left = 2
        lbl.state = uiconst.UI_DISABLED
        uiutil.SetOrder(slider, 0)
        self.sr.dirscroll = uicls.Scroll(name='dirscroll', parent=directionBox)
        ...
        uthread.new(self.ApplyTrackingOnStart_Thread)
        self.LoadProbeList()
        if self.destroyed:
            return
    ...
    def _OnClose(self, *args):
        ...
        if wnd and self.rememberToTurnOffTracking and wnd.trackSelectedItem:
            wnd.ToggleTrackSelectedItem(forceToggle=True)
            self.rememberToTurnOffTracking = False
        sm.GetService('camera').ToggleCenterTrackingPoint(False)
        self.SetMapAngle(0)
    ...
    def GetFilterMenu(self, *args):
        ...
        m = [(uiutil.MenuLabel('UI/Inflight/Scanner/CreateNewFilter'), self.AddFilter)]
        if filterName and filterData:
            m.append((uiutil.MenuLabel('UI/Inflight/Scanner/EditCurrentFilter'), self.EditCurrentFilter))
            m.append((uiutil.MenuLabel('UI/Inflight/Scanner/DeleteCurrentFilter'), self.DeleteCurrentFilter))
        scanSvc = sm.GetService('scanSvc')
        if len(scanSvc.resultsIgnored) > 0:
            m.append(None)
            m.append((uiutil.MenuLabel('UI/Inflight/Scanner/ClearAllIgnoredResults'), scanSvc.ClearIgnoredResults))
            ids = sorted(scanSvc.resultsIgnored)
            m.append((uiutil.MenuLabel('UI/Inflight/Scanner/ClearIgnoredResult'), [ (id, scanSvc.ShowIgnoredResult, (id,)) for id in ids ]))
        return m
    ...
    def DirectionSearch(self, *args):
        ...
        result = None
        try:
            result = sm.GetService('scanSvc').ConeScan(self.scanangle, rnge * 1000, vec[0], vec[1], vec[2])
    ...
    def ShowDirectionalSearchResult(self, *args):
        ...
        if self.scanresult and len(self.scanresult):
            ...
            for slimItem, ball, celestialRec in self.scanresult:
                ...
                scrolllist.append(listentry.Get('DirectionalScanReults', data=data))
                blue.pyos.BeNice()
    ...
    def UseTrackingChanged(self, checked):
        wnd = form.ActiveItem.GetIfOpen()
        if wnd:
            if self.sr.usetracking.checked:
                if not wnd.trackSelectedItem:
                    self.rememberToTurnOffTracking = True
                wnd.TrackSelectedItem()
            if not self.sr.usetracking.checked:
                if self.rememberToTurnOffTracking and wnd.trackSelectedItem:
                    wnd.ToggleTrackSelectedItem(forceToggle=True)
                    self.rememberToTurnOffTracking = False
        settings.user.ui.Set('scannerusestrackingcamera', self.sr.usetracking.checked)
        sm.GetService('camera').ToggleCenterTrackingPoint(self.sr.usetracking.checked)

    def ApplyTrackingOnStart_Thread(self):
        sceneManager = sm.GetService('sceneManager')
        while sceneManager.GetRegisteredCamera('default') is None:
            blue.pyos.synchro.Sleep(100)

        self.UseTrackingChanged(self.sr.usetracking)
    ...
    def ResultMenu(self, panel, *args):
        ...
        if result.certainty >= const.probeResultPerfect and isinstance(result.data, Vector3):
            ...
            menu.append((uiutil.MenuLabel('UI/Inflight/BookmarkLocation'), sm.GetService('addressbook').BookmarkLocationPopup, (eve.session.solarsystemid,
              None,
              None,
              None,
              bookmarkData)))
        ...
        menu.append((uiutil.MenuLabel('UI/Inflight/Scanner/IngoreResult'), scanSvc.IgnoreResult, idList))
        menu.append((uiutil.MenuLabel('UI/Inflight/Scanner/IgnoreOtherResults'), scanSvc.IgnoreOtherResults, idList))
        menu.append((uiutil.MenuLabel('UI/Inflight/Scanner/ClearResult'), scanSvc.ClearResults, idList))
        return menu
    ...
    def CreatePointResult(self, result):
        ...
        if result.certainty >= const.probeResultPerfect:
            ...
            resultBracket = xtriui.SimpleBracket()
...
class DirectionalScanReults(listentry.Generic):
    __guid__ = 'listentry.DirectionalScanReults'
    isDragObject = True
...
class WarpableResultBracket(xtriui.SimpleBracket):
    ...
    def GetMenu(self):
        ...
        menu.append((uiutil.MenuLabel('UI/Inflight/BookmarkLocation'), sm.GetService('addressbook').BookmarkLocationPopup, (eve.session.solarsystemid,
          ...
          bookmarkData)))
        menu.append(None)
        menu.append((uiutil.MenuLabel('UI/Inflight/Scanner/IngoreResult'), scanSvc.IgnoreResult, (self.result.id,)))
        menu.append((uiutil.MenuLabel('UI/Inflight/Scanner/IgnoreOtherResults'), scanSvc.IgnoreOtherResults, (self.result.id,)))
        menu.append((uiutil.MenuLabel('UI/Inflight/Scanner/ClearResult'), scanSvc.ClearResults, (self.result.id,)))
        return menu


class ProbeBracket(xtriui.SimpleBracket):
...
class ProbeControl(object):
    ...
    def __init__(self, probeID, probe, parent, scanner):
        ...
        val = math.sin(mathUtil.DegToRad(45.0)) * 0.2
