#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/shared/comtool/lscengine.py
import service
import types
import copy
import util
...
class LargeScaleChat(service.Service):
    ...
    def Invite(self, otherID, channelID = None, isRecruiting = None):
        ...
        if isRecruiting is not None:
            ...
            sm.GetService('corp').LogCorpRecruitmentEvent('TalkToRecruiter', session.corpid, session.allianceid, isRecruiting.recruiterID, isRecruiting.corporationID, isRecruiting.adID)
        if channelID:
    ...
    def Settings(self, channelID):
        if not self.IsOperator(channelID):
            if isinstance(channelID, tuple):
                ...
                return
        form.ChannelSettingsDialog.Open(channelID=channelID, windowID='ChannelSettingsDialog_%s' % repr(channelID))
    ...
    def __ConfirmInvitation(self, invitorID, invitorName, invitorGender, channelID):
        ...
        if sm.GetService('addressbook').IsInAddressBook(invitorID, 'contact'):
            return
        if form.ChatInviteWnd.GetIfOpen(windowID='ChatInvitation_%s' % invitorID):
            return 'ChtAlreadyInvited'
        if channelID in self.channels:
            return 'ChtAlreadyInChannel'
        if currentcall:
            currentcall.UnLockSession()
        wnd = form.ChatInviteWnd.Open(windowID='ChatInvitation_%s' % invitorID, invitorID=invitorID, invitorName=invitorName)
        if wnd.OnScale_:
            wnd.OnScale_(wnd)
...
class ChannelSettingsDialog(uicls.Window):
    ...
    def OnAllowedTab(self):
        if not self.sr.Get('allowed_initialized', 0):
            ...
            self.sr.allow = uicls.Button(parent=btnParent, label=localization.GetByLabel('UI/Chat/AddToAllowedList'), func=self.__Allow, align=uiconst.CENTER)
    ...
    def OnBlockedTab(self):
        if not self.sr.Get('blocked_initialized', 0):
            ...
            self.sr.allow = uicls.Button(parent=btnParent, label=localization.GetByLabel('UI/Chat/AddToBlockedList'), func=self.__Block, align=uiconst.CENTER)
    ...
    def OnOperatorsTab(self):
        if not self.sr.Get('operators_initialized', 0):
            ...
            self.sr.allow = uicls.Button(parent=btnParent, label=localization.GetByLabel('UI/Chat/AddToOperatorList'), func=self.__Op, align=uiconst.CENTER)
