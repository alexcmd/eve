#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/shared/neocom/journal.py
import service
import uix
import uiutil
import xtriui
        ...
        try:
            if not hasattr(self, 'agentjournal') or self.agentjournal is None:
                ...
                for mission in self.agentjournal[0]:
                    for b in mission[6]:
                        b.hint = localization.GetByLabel(b.hint, location=b.locationID, index=b.locationNumber + 1)
...
class JournalWindow(uicls.Window):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.sr.agentEnglishMissionsCnt = uicls.Container(name='agentEnglishMissionCnt', parent=self.sr.main, padding=(const.defaultPadding,) * 4, height=20, align=uiconst.TOBOTTOM, state=uiconst.UI_HIDDEN)
        self.sr.agentEnglishMissions = uicls.Checkbox(parent=self.sr.agentEnglishMissionsCnt, name='agentMissionEnglish', text=localization.GetByLabel('UI/Journal/JournalWindow/Agents/ShowEnglishMissionNames'), checked=settings.user.localization.Get(ENGLISH_MISSIONS_SETTING, False), callback=self.OnAgentEnglishCheckBoxChange)
    ...
    def LoadIncursionEncounters(self):
        levels = {'scout': localization.GetImportantByLabel('UI/Incursion/Common/Scout'),
    ...
    def GetEncounterSubContent(self, data, *args):
        ...
        for encounter in data.encounters:
            data = {'label': localization.GetImportantByLabel(encounter.name),
    ...
    def ShowAgentTab(self, statusflag = -1):
        ...
        self.sr.agenttabs.Show()
        self.ShowLoad()
        showEnglishTitles = localizationUtil.GetLanguageID() != localization.LOCALE_SHORT_ENGLISH and settings.user.localization.Get(ENGLISH_MISSIONS_SETTING, False)
        if statusflag == 0:
                ...
                for i in xrange(len(missions)):
                    ...
                    if isinstance(missionNameID, (int, long)):
                        ...
                        if showEnglishTitles:
                            enMissionName = localization.GetByMessageID(missionNameID, languageID=localization.LOCALE_SHORT_ENGLISH)
                    else:
            ...
            if len(research):
                for i in xrange(len(research)):
                    ...
                    text = '<t>'.join([cfg.eveowners.Get(agentID).name,
                     ...
                     localizationUtil.FormatNumeric(points, decimalPlaces=2, useGrouping=True),
                     localizationUtil.FormatNumeric(ppd, decimalPlaces=2, useGrouping=True),
                ...
                if status == 0:
                    ...
                    if c.status == const.conStatusRejected:
                        ...
                        if c.dateAccepted + DAY * c.numDays < blue.os.GetWallclockTime() + 6 * HOUR:
                            color = '0xffdd5500'
                        timeRemaining = c.dateAccepted + DAY * c.numDays - blue.os.GetWallclockTime()
