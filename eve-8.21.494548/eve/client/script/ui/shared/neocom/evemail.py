#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/shared/neocom/evemail.py
import blue
import uthread
import uix
import uiutil
...
class MailForm(uicls.Container):
    ...
    def DrawStuff(self, *args):
        ...
        readingContHeightProporations = settings.user.ui.Get('evemail_readingContProporations', 0.5)
        self.sr.rightCont = uicls.DividedContainer(name='rightCont', settingName='evemail_readingContProporations', parent=self.sr.main, align=uiconst.TOALL, victimAlignment=uiconst.TOBOTTOM, victimValueRatio=readingContHeightProporations, pos=(0,
         0,
         const.defaultPadding,
         const.defaultPadding), minVictimValue=26, minRestValue=26)
        self.sr.divider = self.sr.rightCont.splitter
        self.sr.readingPaneCont = self.sr.rightCont.victimCont
        self.sr.readingPane = uicls.EditPlainText(setvalue='', parent=self.sr.rightCont.victimCont, align=uiconst.TOALL, readonly=1)
        self.sr.msgScroll = uicls.Scroll(name='msgScroll', parent=self.sr.rightCont.restCont)
    ...
    def RefreshMsgScrollHeaders(self, headers, tabs = []):
        uicls.Scroll.DrawHeaders(self.sr.msgScroll, headers, tabs)
...
class NewNewMessage(uicls.Window):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.sr.subjecField = subjecField = uicls.SinglelineEdit(name='subjecField', parent=editCont, maxLength=const.mailMaxSubjectSize, pos=(0, 10, 0, 0), label='', align=uiconst.TOTOP)
    ...
    def _OnClose(self, *args):
        self.messageedit = None
        self.parsingReceivers = 0
        wnd = form.CharacterSearchWindow.GetIfOpen()
    ...
    def OnDropReceiver(self, dragObj, nodes):
        for node in nodes:
            if node.__guid__ in uiutil.AllUserEntries():
                self.AddCharacterReceiver(node.charID)
            elif node.Get('__guid__', None) == 'TextLink' and node.Get('url', '').startswith('showinfo'):
    ...
    def GetSearchWnd(self, input = ''):
        ...
        wnd = form.CharacterSearchWindowMail.Open(actionBtns=actionBtn, caption=caption, input=input, showContactList=True, extraIconHintFlag=extraIconHintFlag, configname=self.configname)
        if wnd is not None:
...
class CharacterSearchWindowMail(form.CharacterSearchWindow):
