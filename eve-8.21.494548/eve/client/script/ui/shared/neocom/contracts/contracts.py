#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/shared/neocom/contracts/contracts.py
import blue
import dbg
import xtriui
import uix
...
class ContractsSvc(service.Service):
    ...
    def NeocomBlink(self):
        ...
        if len(kv.needsAttention) > 0:
            sm.GetService('neocom').Blink(form.Journal.default_windowID, localization.GetByLabel('UI/Contracts/ContractsWindow/ContractsRequireAttention'), numBlinks=10)
    ...
    def AcceptContract(self, contractID, forCorp):
        ...
        if c.type == const.conTypeAuction:
            ...
            for item in contract.items:
                if item.inCrate:
                    reportGet += '<t>%s.<br>' % cfg.FormatConvert(TYPEIDANDQUANTITY, item.itemTypeID, max(1, item.quantity))
                else:
                    reportPay += '<t>%s.<br>' % cfg.FormatConvert(TYPEIDANDQUANTITY, item.itemTypeID, max(1, item.quantity))
    ...
    def FailContract(self, contractID):
        ...
        if c.acceptorID == eve.session.charid or c.acceptorID == eve.session.corpid:
            ...
            if blue.os.GetWallclockTime() > c.dateAccepted + DAY * c.numDays and c.status == const.conStatusInProgress:
        ...
        try:
            ...
            for c in contracts:
                ...
                if filtStatus == const.conStatusOutstanding:
                    ...
                    timeLeftValue = c.dateAccepted + DAY * c.numDays - blue.os.GetWallclockTime()
...
class CreateContract(uicls.Window):
        ...
        try:
            ...
            if n == 0:
                ...
                if self.data.type == const.conTypeAuction:
                    ...
                    top = 16
                    self.reqItemTypeWnd = c = uicls.SinglelineEdit(name='itemtype', parent=reqItemsParent, label=localization.GetByLabel('UI/Contracts/ContractsWindow/ItemType'), align=uiconst.TOPLEFT, width=248, isTypeField=True)
                    ...
                    uicls.Line(parent=self.formWnd, align=uiconst.TOTOP, padTop=10)
                    self.sr.courierHint = uicls.EveLabelSmall(name='courierHint', text='', align=uiconst.TOTOP, parent=self.formWnd, state=uiconst.UI_DISABLED, padding=6)
            ...
            if step == 0:
                ...
                if self.data.avail == 1:
                    if IsSearchStringLongEnough(self.data.name):
                        ownerID = uix.Search(self.data.name.lower(), const.groupCharacter, const.categoryOwner, hideNPC=1, filterGroups=[const.groupCharacter, const.groupCorporation, const.groupAlliance], searchWndName='contractFinishStepSearch', hideDustChars=True)
                    if not ownerID:
                        return False
                    if self.data.type == const.conTypeAuction:
    ...
    def WriteSelectItems(self, title):
        ...
        selectBtn = uicls.ButtonGroup(btns=[[localization.GetByLabel('UI/Common/SelectAll'),
          self.SelectAll,
          (),
...
class ContractDetailsWindow(uicls.Window):
    ...
    def Init(self):
        ...
        if c.type == const.conTypeAuction:
            ...
            infoParent = uicls.Container(name='infocontainer', parent=self.sr.main, align=uiconst.TOTOP, width=const.defaultPadding)
            info = uicls.EveLabelMedium(text=text, parent=infoParent, top=const.defaultPadding, idx=0, tabs=tabs, state=uiconst.UI_NORMAL, left=const.defaultPadding)
    ...
    def InsertItemList(self, contract, title, inCrate, numRows, hint = None, fixedSize = False, forceList = False):
        ...
        if len(items) == 0:
            ...
            if len(items) == 1 and not forceList:
                ...
                self.caption = uicls.EveCaptionSmall(parent=infoParent, text=captionText, padRight=16)
