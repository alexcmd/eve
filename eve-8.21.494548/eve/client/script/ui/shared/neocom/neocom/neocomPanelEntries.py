...
class PanelEntryBase(uicls.Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.icon = uicls.Icon(parent=self.main, name='icon', state=uiconst.UI_DISABLED, icon=self.GetIconPath(), size=self.height - 4, left=10, top=2, ignoreSize=True)
    ...
    def GetIconPath(self):
        return self.btnData.iconPath or neocom.ICONPATH_DEFAULT
...
class PanelEntryGroup(PanelEntryBase):
    ...
    def OnDropData(self, source, dropData):
        if not sm.GetService('neocom').IsValidDropData(dropData):
            return
        btnData = dropData[0]
        if btnData.btnType not in neocom.FIXED_PARENT_BTNTYPES:
