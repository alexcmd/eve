#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/shared/manufacturing.py
import blue
import xtriui
import uix
import uiutil
...
DETAILCONTAINERHEIGHT = 68

class ManufacturingSvc(service.Service):
    __exportedcalls__ = {}
    __guid__ = 'svc.manufacturing'
    __notifyevents__ = ['OnSessionChanged', 'OnItemChange', 'OnSolarSystemLPChange']
    ...
    def OnSolarSystemLPChange(self, oldpoints, newpoints):
        oldlevel = facwarCommon.GetLPUpgradeLevel(oldpoints)
        newlevel = facwarCommon.GetLPUpgradeLevel(newpoints)
        if oldlevel != newlevel:
            self.LogInfo('Level changed from', oldlevel, 'to', newlevel, '- updating assembly lines')
            sm.GetService('objectCaching').InvalidateCachedMethodCall('ramProxy', 'AssemblyLinesSelectPublic')
            if session.stationid is not None:
                sm.GetService('objectCaching').InvalidateCachedMethodCall('ramProxy', 'AssemblyLinesGet', session.stationid)
            self.detailsByRegionByTypeFlag = {}
            wnd = form.Manufacturing.GetIfOpen()
            if wnd:
                wnd.ReloadTabs()
    ...
    def GetRegionDetails(self, typeFlag = 'region'):
        ...
        if details is None:
            ...
            return
        newHeader = blue.DBRowDescriptor((('containerID', const.DBTYPE_I8),
         ('containerTypeID', const.DBTYPE_I4),
         ('containerLocationID', const.DBTYPE_I8),
         ('assemblyLineTypeID', const.DBTYPE_UI1),
         ('quantity', const.DBTYPE_UI1),
         ('ownerID', const.DBTYPE_I4),
         ('activityID', const.DBTYPE_I4)))
        newDetails = dbutil.CRowset(newHeader, [])
        for row in details:
            rowList = [ c for c in row ]
            rowList.append(cfg.ramaltypes.Get(row.assemblyLineTypeID).activityID)
            newRow = blue.DBRow(newHeader, rowList)
            newDetails.append(newRow)

        self.detailsByRegionByTypeFlag[regionid][typeFlag] = newDetails
        prime = []
        for line in details:
    ...
    def GetQuoteDialog(self, quoteData):
        ...
        if ownerInput == session.charid:
            if flagInput in (const.flagCargo, const.flagFleetHangar, const.flagSpecializedOreHold):
            ...
            if bomLocationData is None:
                raise UserError('RamNotYourItemToInstall')
        installedItemLocationData = self.GetPathToItem(quoteData.blueprint)
    ...
    def GetPathToItem(self, item):
        ...
        if item.flagID in (const.flagCargo, const.flagFleetHangar):
            if util.IsStation(eve.session.locationid):
...
class BlueprintData():
    ...
    def GetLocationData(self, solarsystemID, station, key, expanded = 0, isCorp = 0, scrollID = None):
        ...
        wnd = form.Manufacturing.GetIfOpen()
        if wnd:
            if hasattr(wnd, 'quickFilter'):
                myFilter = wnd.quickFilter.GetValue()
                if len(myFilter):
                    myData = util.KeyVal()
                    myData.solarsystemID = solarsystemID
                    myData.location = location
                    myData.officeID = getattr(station, 'officeID', None)
                    myData.isCorp = isCorp
                    myData.scrollID = scrollID
                    subContent = self.GetSubContent(myData)
                    if len(subContent) < 1:
                        return []
    ...
    def GetSubContent(self, data, *args):
        ...
        for each in items:
            ...
            if lprr == -1:
                lprr = ''
            copyLabel = localization.GetByLabel('UI/ScienceAndIndustry/Copy')
        ...
        if wnd:
            if hasattr(wnd, 'quickFilter'):
                myFilter = wnd.quickFilter.GetValue()
                if len(myFilter):
                    scrolllist = uiutil.NiceFilter(wnd.quickFilter.QuickFilter, scrolllist)
        return scrolllist
...
class Manufacturing(uicls.Window, BlueprintData):
    ...
    default_width = 635
    default_height = 500
    default_windowID = 'scienceandindustry'
    ...
    def ShowBlueprints(self, isCorp = 0):
        ...
        if not getattr(self, '%sBlueprintsInited' % key, 0):
            ...
            filters = uicls.Container(name='filters', parent=bpParent, align=uiconst.TOTOP)
            self.quickFilter = uicls.QuickFilterEdit(parent=filters, align=uiconst.BOTTOMRIGHT, left=const.defaultPadding)
            self.quickFilter.ReloadFunction = lambda : self.ShowBlueprints(isCorp)
            ...
            c = uicls.Combo(label=typeLabel, parent=filters, options=copies, name='copies', select=settings.user.ui.Get('%s_blueprint_copyfilter' % key, None), callback=self.BlueprintComboChange, left=const.defaultPadding, align=uiconst.BOTTOMLEFT)
            setattr(self.sr, '%s_blueprintCopyFilter' % key, c)
            labelWidth, labelHeight = uicls.EveLabelSmall.MeasureTextSize(typeLabel)
            filters.height = max(self.quickFilter.height, c.height + labelHeight + const.defaultPadding)
            if isCorp:
                ...
                c = uicls.Combo(label=divisionLabel, parent=filters, options=divisions, name='divisions', select=settings.user.ui.Get('blueprint_divisionfilter', (None, None)), callback=self.BlueprintComboChange, left=self.sr.corp_blueprintCopyFilter.left + self.sr.corp_blueprintCopyFilter.width + 4, align=uiconst.BOTTOMLEFT)
            ...
            par = uicls.Container(name='infoParent', parent=bpParent, align=uiconst.TOBOTTOM, height=DETAILCONTAINERHEIGHT, idx=0)
            ...
            if rmjLimit == -1 and rrjLimit == -1:
                ...
                if rmjLimit == -1:
                    ...
                    rmjText = localization.GetByLabel('UI/ScienceAndIndustry/ScienceAndIndustryWindow/LimetedToJumps', jumps=rmjLimit)
                if rrjLimit == -1:
                    ...
                    rrjText = localization.GetByLabel('UI/ScienceAndIndustry/ScienceAndIndustryWindow/LimetedToJumps', jumps=rrjLimit)
        ...
        for solarsystemID, station in sortlocations:
            if station.blueprintCount:
                myLocations = self.GetLocationData(solarsystemID, station, 'allitems', isCorp=isCorp, scrollID=scroll.sr.id)
                if len(myLocations):
                    scrolllist.append(listentry.Get('Group', myLocations))
    ...
    def ShowJobs(self):
        if not getattr(self, 'jobsinited', 0):
            details = uicls.Container(name='details', parent=self.sr.jobsParent, height=80, align=uiconst.TOBOTTOM, idx=1)
    ...
    def GetJobLineMenu(self, entry):
        ...
        if entry.sr.node.jobData:
            if util.IsStation(entry.sr.node.jobData.installedItemLocationID):
                itemID = entry.sr.node.jobData.installedItemLocationID
            elif util.IsStation(entry.sr.node.jobData.containerID):
                itemID = entry.sr.node.jobData.containerID
            else:
                itemID = entry.sr.node.jobData.installedInSolarSystemID
            typeID = entry.sr.node.jobData.containerTypeID
            locationCmdLabel = uiutil.MenuLabel('UI/ScienceAndIndustry/ScienceAndIndustryWindow/LocationCommand')
            m += [(uiutil.MenuLabel('UI/Commands/ShowInfo'), self.ShowInfo, (entry.sr.node.id, None))]
            m += [None]
            ...
            blueprintLabel = uiutil.MenuLabel('UI/ScienceAndIndustry/ScienceAndIndustryWindow/Blueprint')
            ...
            for characterData in menuCharacters:
                tmp = uiutil.MenuLabel('UI/ScienceAndIndustry/ScienceAndIndustryWindow/JobLineMenuUser', {'char': characterData[0],
                 'role': characterData[2]})
                menufunc = None
    ...
    def GetStateTextOrID(self, jobdata):
        ...
        if now >= jobdata.endProductionTime:
            ...
            if now < jobdata.beginProductionTime:
                pendingLabel = localization.GetByLabel('UI/ScienceAndIndustry/ScienceAndIndustryWindow/Pending')
    ...
    def ShowJobDetails(self, jobdata):
        ...
        textWidth, textHeight = uicls.Label.MeasureTextSize(activityLabel, width=col1)
        t = uicls.EveLabelMedium(text='%s<t><right>%s' % (activityLabel, sm.GetService('manufacturing').GetActivityName(jobdata.activityID)), parent=self.sr.details, left=left, top=0, maxLines=1, tabs=[col1, col2])
        t = uicls.EveLabelMedium(text='%s<t><right>%s' % (stateLabel, stateText), parent=self.sr.details, left=left, top=textHeight, maxLines=None, tabs=[col1, col2])
        t = uicls.EveLabelMedium(text='%s<t><right>%s' % (outputLabel, self.GetJobLocationName(jobdata.containerID, jobdata.containerTypeID, jobdata.containerLocationID)), parent=self.sr.details, left=left, top=textHeight * 3, maxLines=1, tabs=[col1, col2])
        ...
        if jobdata.activityID != const.ramActivityReverseEngineering:
            ...
            t = uicls.EveLabelMedium(text='%s<t><right>%s' % (outputTypeLabel, text), parent=self.sr.details, left=left, top=textHeight * 4, maxLines=1, tabs=[col1, col2])
            left = +col2
            col3 = 100
            col4 = 160
            if jobdata.activityID == const.ramActivityResearchingTimeProductivity:
                ...
                t = uicls.EveLabelMedium(text='%s<t><right>%s' % (installPELabel, jobdata.installedItemProductivityLevel), parent=self.sr.details, left=left, top=0, maxLines=1, tabs=[col3, col4])
                t = uicls.EveLabelMedium(text='%s<t><right>%s' % (endPELabel, jobdata.installedItemProductivityLevel + jobdata.runs), parent=self.sr.details, left=left, top=textHeight, maxLines=1, tabs=[col3, col4])
            if jobdata.activityID == const.ramActivityResearchingMaterialProductivity:
                ...
                t = uicls.EveLabelMedium(text='%s<t><right>%s' % (installMELabel, jobdata.installedItemMaterialLevel), parent=self.sr.details, left=left, top=0, maxLines=1, tabs=[col3, col4])
                t = uicls.EveLabelMedium(text='%s<t><right>%s' % (endMELabel, jobdata.installedItemMaterialLevel + jobdata.runs), parent=self.sr.details, left=left, top=textHeight, maxLines=1, tabs=[col3, col4])
            if jobdata.activityID == const.ramActivityCopying:
                ...
                t = uicls.EveLabelMedium(text='%s<t><right>%s' % (copiesLabel, jobdata.runs), parent=self.sr.details, left=left, top=0, maxLines=1, tabs=[col3, 160])
                t = uicls.EveLabelMedium(text='%s<t><right>%s' % (productionRunsLabel, jobdata.licensedProductionRuns), parent=self.sr.details, left=left, top=textHeight, maxLines=1, tabs=[col3, col4])
        self.sr.detailtext = uicls.EveLabelMedium(text='', parent=self.sr.details, left=3, top=textHeight * 2, maxLines=1, tabs=[130, col2])
        uicls.Line(parent=self.sr.details, align=uiconst.RELATIVE, width=1, height=textHeight * 5, left=col2)
        ...
        self.sr.details.height = textHeight * 5.5
...
class InstallationPanel(uicls.Container):
    ...
    def Init(self, activityID = None, stationID = None):
        ...
        self.sr.installationDetails = uicls.Container(name='installationDetails', parent=self, height=DETAILCONTAINERHEIGHT, align=uiconst.TOBOTTOM, idx=0)
        self.inited = 1
    ...
    def ShowAssemblyLines(self, assemblyLine):
        ...
        t = uicls.EveLabelMedium(text=selectAssemblyLabel, parent=self.sr.installationDetails, left=10, top=30, maxLines=1, tabs=[128, 170])
        if scrolllist:
    ...
    def ShowAssemblyLineDetails(self, entry):
        ...
        col1 = 165
        col2 = 205
        maxHeight = DETAILCONTAINERHEIGHT
        lines = []
        for header, key in d:
            if key not in entry.sr.node.assemblyLine.__columns__:
                continue
            value = entry.sr.node.assemblyLine[key]
            value = localizationUtil.FormatNumeric(value)
            t = uicls.EveLabelMedium(text='%s<t><right>%s' % (header, value), parent=self.sr.installationDetails, left=left, top=top, maxLines=1, tabs=[col1, col2], hint=header)
            maxHeight = max(maxHeight, t.top + t.height)
            i += 1
            if i == 4:
                left += col2 + 8
                line = uicls.Line(parent=self.sr.installationDetails, align=uiconst.TOPLEFT, width=1, top=5, height=64, left=left - 7)
                lines.append(line)
                i = 0
                top = 5
            else:
        ...
        if i != 0:
            line = uicls.Line(parent=self.sr.installationDetails, align=uiconst.RELATIVE, width=1, top=5, height=64, left=left + col2)
            lines.append(line)
        ...
        if self.CanManageAssemblyLines():
            ...
            submit = uicls.Button(parent=self.sr.installationDetails, label=manageAssebmlyLinesLabel, func=self.ManageAssemblyLines, pos=(0,
             ...
             0,
             0), align=uiconst.TOPRIGHT)
        self.sr.installationDetails.height = maxHeight
        for line in lines:
            line.height = maxHeight - line.top
    ...
    def AssemblyLineRow(self, data):
        ...
        data.label = '%s<t>%s%s<t><right>%s<t><right>%s<t><right>%s<t><right>%s<t>%s' % (localizationUtil.FormatNumeric(data.numLines, decimalPlaces=0),
         ...
         localizationUtil.FormatNumeric(lineType.baseTimeMultiplier, decimalPlaces=2),
         localizationUtil.FormatNumeric(lineType.baseMaterialMultiplier, decimalPlaces=2),
         rmText)
    ...
    def ManageAssemblyLines(self, entry):
        ...
        if retval:
            ...
            for line, lineType, lineGroup, lineCategory in selectedAssemblyLineDetails:
                if rowset is None:
                    rowDescriptor = line.__header__
                    rowset = dbutil.CRowset(rowDescriptor, [])
                rowset.InsertNew(line)

            header = rowset.columns
        ...
        sm.services['objectCaching'].InvalidateCachedMethodCall('ramProxy', 'AssemblyLinesGet', entry.containerID)
        self.ShowAssemblyLines(entry)
...
class InstallationWindow(uicls.Window):
    ...
    def GetInputOutputLocations(self, isMine = True, locationID = None):
        ...
        if isMine:
            if self.sr.assemblyLine and self.sr.assemblyLine.containerID == eve.session.shipid:
                ...
                typeObj = godma.GetType(ship.typeID)
                ...
                IOOpts = [(myCargoLabel, (eve.session.charid, const.flagCargo))]
                if bool(typeObj.hasFleetHangars):
                    myFHLabel = localization.GetByLabel('UI/Ship/FleetHangar')
                    IOOpts.append((myFHLabel, (eve.session.charid, const.flagFleetHangar)))
                if bool(sm.GetService('godma').GetType(ship.typeID).specialOreHoldCapacity):
                    myOreHoldLabel = localization.GetByLabel('UI/Ship/OreHold')
                    IOOpts.append((myOreHoldLabel, (eve.session.charid, const.flagSpecializedOreHold)))
                containerIptOps = containerOptOps = IOOpts
            else:
    ...
    def CheckInventionItems(self):
        ...
        if details.activityID == const.ramActivityInvention:
            ...
            itemOptions.sort(lambda x, y: cmp(x[0], y[0]))
        ...
        decryptorOptions.sort(lambda x, y: cmp(x[0], y[0]))
        ...
        selectedDecryptor = self.sr.inventionDecryptorCombo.GetValue()
        self.sr.inventionDecryptorCombo.LoadOptions(decryptorOptions, selectedDecryptor)
        if outputTypeIDs:
            for typeID in outputTypeIDs:
                name = cfg.invtypes.Get(typeID).name
                outputTypes.append((name, typeID))

            selectedBaseItem = self.sr.inventionBaseItemCombo.GetValue()
            self.sr.inventionBaseItemCombo.LoadOptions(itemOptions, selectedBaseItem)
            selectedOutputType = self.sr.inventionOutputCombo.GetValue()
            outputTypes.sort(lambda x, y: cmp(x[0], y[0]))
            self.sr.inventionOutputCombo.LoadOptions(outputTypes, selectedOutputType)
...
class PickBlueprintWindow(form.ListWindow, BlueprintData):
    ...
    def LoadBlueprints(self, isMine = 1):
        ...
        if not getattr(self, '%sInited' % key, None):
            ...
            setattr(self.sr, '%s_blueprintCopyFilter' % key, c)
            if isCorp:
                divisions = sm.GetService('manufacturing').GetAvailableHangars(canView=1, canTake=0)
                divisions.insert(0, (allLabel, (None, None)))
                divisionLabel = localization.GetByLabel('UI/ScienceAndIndustry/ScienceAndIndustryWindow/Division')
...
class PlanetPanel(uicls.Container):
    ...
    def GetPlanetScrollList(self):
        ...
        for row in rows:
            ...
            data.Set('sort_%s' % localization.GetByLabel('UI/ScienceAndIndustry/ScienceAndIndustryWindow/PlanetName'), (cfg.evelocations.Get(row.solarSystemID).name, row.celestialIndex))
            scrolllist.append(listentry.Get('Generic', data=data))
    ...
    def GetPlanetEntryMenu(self, entry):
        ...
        if node.solarSystemID != session.solarsystemid:
            ...
            if eve.session.role & (service.ROLE_GML | service.ROLE_WORLDMOD):
                gmExtrasLabel = uiutil.MenuLabel('UI/ScienceAndIndustry/ScienceAndIndustryWindow/GMWMExtrasCommand')
            ...
            if isOpen:
                menu += [[uiutil.MenuLabel('UI/PI/Common/ExitPlanetMode'), sm.GetService('viewState').CloseSecondaryView, ()]]
            else:
                openPlanet = lambda planetID: sm.GetService('viewState').ActivateView('planet', planetID=planetID)
                menu += [(uiutil.MenuLabel('UI/PI/Common/ViewInPlanetMode'), sm.GetService('planetUI').Open, (node.planetID,))]
            menu += [(uiutil.MenuLabel('UI/Commands/ShowInfo'), menuSvc.ShowInfo, (node.typeID,
               node.planetID,
               0,
               None,
               None))]
