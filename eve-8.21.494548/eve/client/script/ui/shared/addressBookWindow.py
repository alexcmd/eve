...
class AddressBookWindow(uicls.Window):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.sr.searchBy = searchBy = uicls.Combo(label=localization.GetByLabel('UI/Common/SearchBy'), parent=self.sr.topParent, options=searchByChoices, name='addressBookComboSearchBy', select=settings.user.ui.Get('ppSearchBy', 1), width=233, left=75, top=inpt.top + inpt.height + 2, labelleft=group.left + group.width + 6 - 75, callback=self.ChangeSearchBy)
    ...
    def Search(self, *args):
        ...
        if groupID == -1:
            ...
            groupIDList = [groupID]
        exactSearch = settings.user.ui.Get('ppSearchBy', const.searchByPartialTerms)
