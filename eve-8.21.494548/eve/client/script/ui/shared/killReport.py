...
class KillReportWnd(uicls.Window):
    ...
    def ConstructLayout(self):
        ...
        self.victimCorpName = uicls.EveLabelMedium(text='', parent=victimCorpTextCont, name='victimCorpName', align=uiconst.TOTOP, state=uiconst.UI_NORMAL, top=const.defaultPadding)
        self.victimAllianceName = uicls.EveLabelMedium(text='', parent=victimCorpTextCont, name='victimAllianceName', align=uiconst.TOTOP, state=uiconst.UI_NORMAL)
        ...
        killersCont = uicls.Container(name='killersCont', parent=bottomLeftCont, align=uiconst.TOALL)
        self.killedOnBehalfCont = uicls.Container(name='killedOnBehalfCont', parent=killersCont, align=uiconst.TOTOP, height=90)
        onBehalfLabel = uicls.EveLabelLarge(text=localization.GetByLabel('UI/Corporations/Wars/Killmails/KilledOnBehalf'), parent=self.killedOnBehalfCont, align=uiconst.TOTOP)
        self.behalfCont = uicls.KilledOnBehalfContainer(name='behalfCont', parent=self.killedOnBehalfCont, align=uiconst.TOTOP)
        ...
        self.damageTaken = uicls.EveLabelMedium(text='', parent=killersCont, name='damageTaken', align=uiconst.TOTOP, color=(1.0, 0.0, 0.0))
        finalBlowCont = uicls.Container(name='finalBlowCont', parent=killersCont, align=uiconst.TOTOP, height=84, top=8)
        finalBlowLabel = uicls.EveLabelMedium(text=localization.GetByLabel('UI/Corporations/Wars/Killmails/FinalBlow'), parent=finalBlowCont, align=uiconst.TOTOP)
        ...
        topDamageLabel = uicls.EveLabelMedium(text=localization.GetByLabel('UI/Corporations/Wars/Killmails/TopDamage'), parent=topDamageCont, align=uiconst.TOTOP)
        ...
        self.savefittingBtn = uicls.Button(label=localization.GetByLabel('UI/Corporations/Wars/Killmails/SaveFitting'), parent=topItemsCont, align=uiconst.TOPRIGHT, func=self.SaveFitting)
        ...
        bottomItemsCont = uicls.ContainerAutoSize(name='bottomItemsCont', parent=itemsCont, align=uiconst.TOBOTTOM)
        ...
        totalLossCont = uicls.Container(name='totalLossCont', parent=bottomItemsCont, align=uiconst.TOTOP, height=24)
        uicls.EveLabelLarge(text=localization.GetByLabel('UI/Corporations/Wars/Killmails/TotalWorth'), parent=totalLossCont, name='totalWorth', align=uiconst.TOPLEFT, top=2)
        self.totalWorthLabel = uicls.EveLabelLarge(text='', parent=totalLossCont, name='totalWorthLabel', align=uiconst.TOPRIGHT, top=2)
        self.totalPayoutCont = uicls.ContainerAutoSize(name='totalPayoutCont', parent=bottomItemsCont, align=uiconst.TOTOP)
        self.pendCont = uicls.Container(name='pendCont', parent=self.totalPayoutCont, align=uiconst.TOTOP, height=14)
        uicls.EveLabelSmall(text=localization.GetByLabel('UI/Corporations/Wars/Killmails/PendInsurance'), parent=self.pendCont, name='totalLoss', align=uiconst.BOTTOMLEFT)
        self.pendLabel = uicls.EveLabelSmall(text='', parent=self.pendCont, name='pendLabel', align=uiconst.BOTTOMRIGHT, color=(1.0, 0.0, 0.0))
        self.lpCont = uicls.Container(name='lpCont', parent=self.totalPayoutCont, align=uiconst.TOTOP, height=14)
        uicls.EveLabelSmall(text=localization.GetByLabel('UI/Corporations/Wars/Killmails/LPPaidOut'), parent=self.lpCont, name='totalLP', align=uiconst.BOTTOMLEFT)
        self.totalLPLabel = uicls.EveLabelSmall(text='', parent=self.lpCont, name='totalLPLabel', align=uiconst.BOTTOMRIGHT, color=(0.0, 1.0, 0.0))
        self.lpCont.display = False
        self.bountyCont = uicls.Container(name='bountyCont', parent=self.totalPayoutCont, align=uiconst.TOTOP, height=14, display=False)
        uicls.EveLabelSmall(text=localization.GetByLabel('UI/Corporations/Wars/Killmails/BountyPaidOut'), parent=self.bountyCont, name='totalBounty', align=uiconst.BOTTOMLEFT)
        self.totalBountyLabel = uicls.EveLabelSmall(text='', parent=self.bountyCont, name='totalBountyLabel', align=uiconst.BOTTOMRIGHT, color=(0.0, 1.0, 0.0))
            ...
            for item in items['other'][const.flagCargo]:
                ...
                if item.contents:
                    for containerItem in item.contents:
                        destroyedData, droppedData = self.MakeItemEntry(containerItem)
                        if destroyedData:
                            scrolllist.append(listentry.Get('KillItems', data=destroyedData))
                        if droppedData:
                            scrolllist.append(listentry.Get('KillItems', data=droppedData))
...
class KillItems(listentry.Generic):
    ...
    def Load(self, node):
        ...
        if categoryID == const.categoryBlueprint:
            self.sr.icon.top = 0
            if singleton == const.singletonBlueprintCopy:
                isCopy = True
...
class KilledOnBehalfContainer(uicls.Container):
    ...
    def ConstructLayout(self):
        uicls.Fill(bgParent=self, color=(1.0, 0.0, 0.0, 0.2))
        uicls.Frame(parent=self, color=(1.0, 0.0, 0.0, 0.2))
        iconCont = uicls.Container(name='iconCont', parent=self, align=uiconst.TOLEFT, width=96)
        self.behalfCont = uicls.Container(name='behalfCont', parent=iconCont, align=uiconst.TOLEFT, width=64, state=uiconst.UI_NORMAL)
        self.corpCont = uicls.Container(name='corpCont', parent=iconCont, align=uiconst.TOPRIGHT, width=32, height=32)
        self.allianceCont = uicls.Container(name='allianceCont', parent=iconCont, align=uiconst.BOTTOMRIGHT, width=32, height=32)
        theRestCont = uicls.Container(name='textCont', parent=self, align=uiconst.TOALL, padLeft=const.defaultPadding)
        self.textCont = uicls.ContainerAutoSize(parent=theRestCont, name='textCont', align=uiconst.CENTERLEFT)
        self.nameLabel = uicls.EveLabelSmall(text='', parent=self.textCont, maxLines=1, align=uiconst.TOPLEFT, state=uiconst.UI_NORMAL)
        self.corpLabel = uicls.EveLabelSmall(text='', parent=self.textCont, maxLines=1, align=uiconst.TOPLEFT, state=uiconst.UI_NORMAL, top=13)
        self.allianceLabel = uicls.EveLabelSmall(text='', parent=self.textCont, maxLines=1, align=uiconst.TOPLEFT, state=uiconst.UI_NORMAL, top=26)
    ...
    def LoadInfo(self, behalfID, corpID, allianceID = None, factionID = None):
        ...
        behalfLogo = uicls.Sprite(parent=self.behalfCont, align=uiconst.TOALL, idx=0, texturePath='res:/UI/Texture/silhouette_64.png')
        if behalfID:
            sm.GetService('photo').GetPortrait(behalfID, 64, behalfLogo)
            behalfLogo.OnClick = (self.ShowInfo, behalfID, const.typeCharacterAmarr)
            behalfLogo.hint = cfg.eveowners.Get(behalfID).name
        if behalfID:
            self.nameLabel.text = localization.GetByLabel('UI/Contracts/ContractsWindow/ShowInfoLink', showInfoName=cfg.eveowners.Get(behalfID).name, info=('showinfo', const.typeCharacterAmarr, behalfID))
        ...
        corpLogo.OnClick = (self.ShowInfo, corpID, const.typeCorporation)
        corpLogo.hint = corpName
        corpLogo.SetSize(32, 32)
        self.corpLabel.text = localization.GetByLabel('UI/Contracts/ContractsWindow/ShowInfoLink', showInfoName=corpName, info=('showinfo', const.typeCorporation, corpID))
        if allianceID:
            allianceName = cfg.eveowners.Get(allianceID).name
            self.allianceLabel.text = localization.GetByLabel('UI/Contracts/ContractsWindow/ShowInfoLink', showInfoName=allianceName, info=('showinfo', const.typeAlliance, allianceID))
            ...
            allianceLogo.OnClick = (self.ShowInfo, allianceID, const.typeAlliance)
            allianceLogo.hint = allianceName
        else:
            self.allianceLabel.text = ''
            allianceLogo = uicls.Sprite(texturePath='res:/UI/Texture/defaultAlliance.dds', parent=self.allianceCont, align=uiconst.TOPLEFT, width=32, height=32, state=uiconst.UI_NORMAL)
