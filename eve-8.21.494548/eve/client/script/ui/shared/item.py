...
class InvItem(uicls.SE_BaseClassCore):
    ...
    def ConstructLockedIcon(self):
        if not self.lockedIcon:
            self.lockedIcon = uicls.Sprite(name='lockedIcon', parent=self.iconCont, texturePath='res:/UI/Texture/classes/Inventory/locked.png', align=uiconst.TOPRIGHT, pos=(0, 0, 13, 14), idx=0, hint=localization.GetByLabel('UI/Inventory/ItemLocked'))
        if not self.darkenedBG:
            self.darkenedBG = uicls.Sprite(name='darkenedBG', parent=self.iconCont, texturePath='res:/UI/Texture/classes/Inventory/darken.png', state=uiconst.UI_DISABLED, align=uiconst.TOALL, opacity=0.3, padding=-1, idx=1)
        ...
        if util.GetActiveShip() == self.sr.node.item.itemID:
            if self.activeShipHighlite is None:
                if listFlag:
                    ...
                    padding = -3
                self.activeShipHighlite = uicls.Container(name='activeShipHighlite', parent=self, idx=-1)
                uicls.Frame(texturePath='res:/UI/Texture/Classes/InvItem/bgSelected.png', color=util.Color.GREEN, parent=self.activeShipHighlite, padding=padding)
                uicls.Frame(parent=self.activeShipHighlite, color=util.Color.GREEN, padding=padding, opacity=0.4)
        elif self.activeShipHighlite:
            self.activeShipHighlite.Close()
            self.activeShipHighlite = None
        if self.sr.node.Get('selected', 0):
            self.ShowSelected(animate=False)
        else:
            self.ShowDeselected(animate=False)
    ...
    def ShowSelected(self, animate = True):
        ...
        if animate:
            ...
            uicore.animations.SpColorMorphTo(self.sr.label, self.sr.label.GetRGBA(), util.Color.WHITE, duration=0.15)
        else:
            self.scrollEntrySelectionHilite.opacity = 1.0
            self.sr.label.SetRGBA(*util.Color.WHITE)

    def ShowDeselected(self, animate = True):
        if self.scrollEntrySelectionHilite is None:
            return
        col = util.Color.WHITE
        if animate:
            uicore.animations.FadeOut(self.scrollEntrySelectionHilite, duration=0.15)
            uicore.animations.SpColorMorphTo(self.sr.label, self.sr.label.GetRGBA(), col, duration=0.15)
        else:
            self.scrollEntrySelectionHilite.opacity = 0.0
            self.sr.label.SetRGBA(*col)
    ...
    def OnDblClick(self, *args):
        if self.sr.node and self.sr.node.Get('OnDblClick', None):
            ...
            if not sm.GetService('menu').CheckSameLocation(self.rec):
                return
    ...
    def OpenShipCargo(self):
        ...
        form.Inventory.OpenOrShow(invID=('ShipCargo', self.rec.itemID), openFromWnd=wnd)
    ...
    def OpenContainer(self):
        ...
        if self.rec.typeID == const.typePlasticWrap:
            form.Inventory.OpenOrShow(invID=('StationContainer', self.rec.itemID), openFromWnd=wnd)
        elif sm.StartService('menu').CheckSameLocation(self.rec):
            invID = ('StationContainer', self.rec.itemID)
            form.Inventory.OpenOrShow(invID=invID, openFromWnd=wnd)
        else:
    ...
    def ConstructBlinkBG(self):
        if self.blinkBG is None:
            self.blinkBG = uicls.Sprite(name='blinkBg', parent=self.iconCont, align=uiconst.TOALL, state=uiconst.UI_DISABLED, texturePath='res:/UI/Texture/classes/InvItem/bgSelected.png', opacity=0.0, idx=0)
...
class Item(InvItem):
    ...
    def ConstructLockedIcon(self):
        if not self.lockedIcon:
            self.lockedIcon = uicls.Sprite(name='lockedIcon', parent=self, texturePath='res:/UI/Texture/classes/Inventory/locked.png', align=uiconst.TOPLEFT, pos=(30, 0, 12, 12), idx=0, hint=localization.GetByLabel('UI/Inventory/ItemLocked'))
