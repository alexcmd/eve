...
class StarMapSvc(service.Service):
    ...
    __dependencies__ = ['pathfinder', 'map']
    ...
    def CleanUp(self):
        ...
        sm.GetService('camera').ClearCameraParent('starmap')
    ...
    def PickParticle(self):
        ...
        particleID = trinity.PickParticle(self.starParticles, x + 1, y + 1, self.mapStars.worldTransform, view, projection, viewport, self.GetStarPickRadius())
        if particleID != -1:
            return particleID
        else:
            return
    ...
    def MakeBroadcastBracket(self, gbType, itemID, charID):
        ...
        anchor = xtriui.Bracket(parent=uicore.layer.starmap, state=uiconst.UI_DISABLED, width=1, height=1, align=uiconst.NOALIGN, name='fleetBroadcastAnchor_%s' % sysname)
    ...
    @telemetry.ZONE_METHOD
    def Unflatten(self):
        registeredScene = sm.GetService('sceneManager').GetRegisteredScene('starmap')
    ...
    def DrawPoints(self, parent):
        ...
        self.SetSolarSystemPoints(solarSystemList, trinity.TriVector(0.0, 0.0, 0.0), 1)
    ...
    def InitMap(self):
        ...
        if self.mapRoot is None:
            ...
            camera.translationFromParent = settings.user.ui.Get('starmapTFP', 0.6 * ZOOM_MAX_STARMAP)
            ...
            sm.GetService('sceneManager').RegisterScene(scene, 'starmap')
    ...
    @telemetry.ZONE_METHOD
    def SetSolarSystemPoints(self, solarSystemList, regionPos, randomizeFrame = 0, sFactor = None, size = None):
        if sFactor is None:
        ...
        for solarsystem in solarSystemList:
            ...
            data.append((pos, SUNBASE, star.color))
            solarSystemIDs.append(solarsystem.itemID)

        self.starParticles.SetData(data)
    ...
    def SetStarParticleAlpha(self, alpha, particleIDs = None):
        ...
        for id in particleIDs:
            ...
            color[3] = alpha
            particleSystem.SetItemElement(id, 2, color)
