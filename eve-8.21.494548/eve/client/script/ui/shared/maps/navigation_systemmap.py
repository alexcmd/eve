#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/shared/maps/navigation_systemmap.py
import trinity
import uthread
import uix
import uiutil
...
class SystemMapLayer(uicls.LayerCore):
    ...
    def OnMouseMove(self, *args):
        ...
        if lib.leftbtn and not lib.rightbtn:
            ...
            camera.Dolly(-(dy * 0.01) * abs(camera.translationFromParent))
            camera.translationFromParent = sm.GetService('camera').CheckTranslationFromParent(camera.translationFromParent, source='systemmap')
