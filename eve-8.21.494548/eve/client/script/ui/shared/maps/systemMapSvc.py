#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/shared/maps/systemMapSvc.py
import service
...
class SystemMapSvc(service.Service):
    ...
    def CleanUp(self):
        ...
        sm.GetService('camera').ClearCameraParent('systemmap')
        ...
        if scannerWnd:
            ...
            for probeID in validProbes:
                ...
                for bracket in bracketWnd.children:
                    ...
                    if not cls or not issubclass(cls, xtriui.SimpleBracket) or issubclass(cls, xtriui.ProbeBracket):
                        continue
    ...
    def LoadSolarsystemBrackets(self, reload = 0):
        ...
        bb = trinity.renderContext.GetDefaultBackBuffer()
        if bb.width & 1:
        ...
        for tf in solarsystem.children:
            ...
            panel = xtriui.Bracket(name='__solarsystembracket', align=uiconst.NOALIGN, state=uiconst.UI_NORMAL, width=16, height=16)
    ...
    def LoadBallparkItems(self, bracketName, bracketPrefix, itemGroupIDs, overrideWidthHeight = None):
        ...
        for itemID, ball in ballpark.balls.iteritems():
            ...
            if slimItem.groupID not in itemGroupIDlist:
                continue
            panel = xtriui.Bracket(name=bracketName, align=uiconst.NOALIGN, state=uiconst.UI_NORMAL, width=16, height=16)
            panel.data = sm.GetService('bracket').GetBracketProps(slimItem, ball)
            try:
                displayName = uix.GetSlimItemName(slimItem)
            except:
...
class BookmarkBracket(xtriui.SimpleBracket):
