#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/shared/fleet/fleet.py
import xtriui
import uix
import mathUtil
import util
...
class FleetHeader(listentry.Group):
    ...
    def Startup(self, *args, **kw):
        ...
        self.sr.myGroupSelection = uicls.Fill(name='myGroupSelection', bgParent=self, padding=(0, 1, 0, 1), color=(0.0, 0.5, 0.0, 0.1))
        self.sr.myGroupSelection.display = False
        self.movingHilite = uicls.Fill(bgParent=self, name='movingHilite', color=(1, 0, 0, 0.25))
    ...
    def Load(self, node, *args, **kw):
        ...
        if node.commanderData and node.commanderData.itemID == session.charid:
            ...
            if isMyGroup:
                self.sr.expander.color = (0.0, 0.5, 0.0, 0.8)
    ...
    def OnDragEnter(self, dragObj, nodes):
        ...
        canMove = sm.GetService('fleet').CanMoveToThisEntry(draggedGuy, self.sr.node, groupType, groupID=groupID)
    ...
    def GetMenu(self):
        ...
        if commanderData:
            ...
            m += sm.GetService('menu').CharacterMenu(commanderData.charID, filterFunc=filterFunc)
        ...
        if self.sr.node.groupType == 'fleet':
            ...
            m += [None, (uiutil.MenuLabel('UI/Fleet/FleetWindow/OpenAllSquads'), self.ToggleAllSquads, (self.sr.node, True)), (uiutil.MenuLabel('UI/Fleet/FleetWindow/CloseAllSquads'), self.ToggleAllSquads, (self.sr.node, False))]
            return m
        raise NotImplementedError
...
class FleetMember(listentry.BaseTacticalEntry):
    ...
    def Startup(self, *args, **kw):
        ...
        self.sr.myGroupSelection = uicls.Fill(name='myGroupSelection', bgParent=self, padding=(0, 1, 0, 1), color=(0.0, 0.5, 0.0, 0.15))
        self.sr.myGroupSelection.display = False
        self.movingHilite = uicls.Fill(bgParent=self, name='movingHilite', color=(1, 0, 0, 0.25))
        self.movingHilite.display = False
    ...
    def OnDragEnter(self, dragObj, nodes):
        ...
        if self.sr.node.itemID == draggedGuy.charID:
            return
        canMove = sm.GetService('fleet').CanMoveToThisEntry(draggedGuy, self.sr.node, 'fleetMember', groupID=None)
        if canMove == CONNOT_BE_MOVED_INCOMPATIBLE:
            return
        if canMove == CANNOT_BE_MOVED:
            self.movingHilite.SetRGB(*COLOR_CANNOT_BE_MOVED)
    ...
    def GetMenu(self):
        ...
        if shipItem is None:
            ...
            ret = [ entry for entry in sm.GetService('menu').CelestialMenu(shipItem.itemID) if entry and entry[0] in (uiutil.MenuLabel('UI/Inflight/LockTarget'),
             uiutil.MenuLabel('UI/Inflight/ApproachObject'),
             uiutil.MenuLabel('UI/Inflight/OrbitObject'),
             uiutil.MenuLabel('UI/Inflight/Submenus/KeepAtRange')) ]
            ret.append(None)
        filterFunc = [uiutil.MenuLabel('UI/Fleet/Fleet'), uiutil.MenuLabel('UI/Commands/ShowInfo')]
        ret.extend(sm.GetService('menu').FleetMenu(self.sr.node.charID, unparsed=False))
        ret.extend(sm.GetService('menu').CharacterMenu(self.sr.node.charID, filterFunc=filterFunc))
        return ret
...
class WatchListPanel(form.ActionPanel):
    ...
    def default_left(*args):
        return uicore.desktop.width - form.WatchListPanel.default_width - 16
    ...
    def DropInWatchList(self, dragObj, entries, idx = None, *args):
        if len(entries) != 1:
            return
        if dragObj.__guid__ == 'listentry.WatchListEntry':
            self.Move(dragObj, entries, idx)
        else:
            if dragObj.__guid__ == 'uicls.Label':
                itemID = self.GetLabelCharID(dragObj, entries)
            elif dragObj.__guid__ in uiutil.AllUserEntries():
                itemID = entries[0].itemID
            else:
                return
            if not FleetSvc().IsFavorite(itemID):
                FleetSvc().AddFavorite(itemID)
                if idx is not None:
                    self.Move(dragObj, entries, idx)
    ...
    def Move(self, dragObj, entries, idx = None, *args):
        ...
        if not charID:
            charID = self.GetLabelCharID(dragObj, entries)
        if not charID:
            return
        if idx is not None:
            if selected:
    ...
    def GetLabelCharID(self, dragObj, entries):
        if dragObj.__guid__ == 'uicls.Label':
            labelInfo = entries[0].url.split('//')
            try:
...
class WatchListEntry(listentry.BaseTacticalEntry):
    ...
    def Startup(self, *args, **kw):
        ...
        labelContainer = uicls.Container(name='labelContainer', parent=self, align=uiconst.TOALL, clipChildren=True)
    ...
    def GetMenu(self):
        ...
        if shipItem is None:
            ...
            ret = [ entry for entry in sm.GetService('menu').CelestialMenu(shipItem.itemID) if entry and entry[0] in (uiutil.MenuLabel('UI/Inflight/LockTarget'),
             uiutil.MenuLabel('UI/Inflight/UnlockTarget'),
             uiutil.MenuLabel('UI/Inflight/ApproachObject'),
             uiutil.MenuLabel('UI/Inflight/OrbitObject'),
             uiutil.MenuLabel('UI/Inflight/Submenus/KeepAtRange')) ]
            ret.append(None)
        ...
        ret += sm.GetService('menu').CharacterMenu(charID, filterFunc=filterFunc)
        return ret
    ...
    def OnDragEnter(self, dragObj, nodes, *args):
        ...
        if not charID and dragObj.__guid__ == 'uicls.DragContainerCore':
            node = nodes[0]
