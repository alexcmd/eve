#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/shared/planet/importExportUI.py
import uiconst
import const
import listentry
import blue
...
class PlanetaryImportExportUI(uicls.Window):
    ...
    def Layout(self):
        ...
        self.sr.customsGauge = uicls.Gauge(parent=self.sr.customsHeader, value=0.0, color=planetCommon.PLANET_COLOR_STORAGE, left=0, top=25, state=uiconst.UI_HIDDEN, align=uiconst.TOPLEFT)
        ...
        self.sr.spaceportGauge = uicls.Gauge(parent=self.sr.spaceportHeader, value=0.0, color=planetCommon.PLANET_COLOR_STORAGE, left=0, top=25, state=uiconst.UI_HIDDEN, align=uiconst.TOPRIGHT)
    ...
    def OpenConfiguration(self):
        form.OrbitalConfigurationWindow.Open(orbitalItem=self.customsOfficeItem)
    ...
    def GetCommodityVolume(self, commodities = None, excluded = None):
        ...
        for key, item in commodities.iteritems():
            if excluded is None or key not in excluded:
                volume += cfg.GetTypeVolume(key[1], item.quantity)
