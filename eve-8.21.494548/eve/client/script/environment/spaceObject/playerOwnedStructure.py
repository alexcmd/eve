...
class PlayerOwnedStructure(spaceObject.LargeCollidableStructure):
    ...
    def LoadModel(self):
        ...
        if self.IsAnchored():
            ...
            spaceObject.SpaceObject.LoadModel(self, 'res:/dx9/Model/deployables/nanocontainer/nanocontainer.red')
    ...
    def PrepareModelForConstruction(self, builtAlready, onlineAlready):
        ...
        if not hasattr(self.model, 'curveSets'):
            return (None, None)
        model = self.model
        lodsAvailable = getattr(model, 'highDetailMesh', None) is not None and getattr(model, 'mediumDetailMesh', None) is not None and getattr(model, 'lowDetailMesh', None) is not None
        alphaParams = []
        if lodsAvailable:
