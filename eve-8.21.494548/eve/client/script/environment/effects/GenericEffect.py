...
class StretchEffect(GenericEffect):
    ...
    def Prepare(self):
        ...
        if shipBall.__guid__ != 'spaceObject.Ship' and shipBall.__guid__ != 'spaceObject.EntityShip':
            self.gfx.source.boundingSize = (shipBall.radius, shipBall.radius, shipBall.radius)
            ...
            if shipBBoxMin is None or shipBBoxMax is None:
                ...
                self.gfx.source.boundingSize = (1.2 * max(-minX, maxX), 1.2 * max(-minY, maxY), 1.2 * max(-minZ, maxZ))
            else:
                self.gfx.source.boundingSize = (1.2 * max(-shipBBoxMin.x, shipBBoxMax.x), 1.2 * max(-shipBBoxMin.y, shipBBoxMax.y), 1.2 * max(-shipBBoxMin.z, shipBBoxMax.z))
        else:
        ...
        if targetBall.__guid__ != 'spaceObject.Ship' and targetBall.__guid__ != 'spaceObject.EntityShip':
            self.gfx.dest.boundingSize = (targetBall.radius, targetBall.radius, targetBall.radius)
            ...
            if shipBBoxMin is None or shipBBoxMax is None:
                ...
                maxY = center[1] + radius
                maxZ = center[2] + radius
                self.gfx.dest.boundingSize = (1.2 * max(-minX, maxX), 1.2 * max(-minY, maxY), 1.2 * max(-minZ, maxZ))
            else:
                self.gfx.dest.boundingSize = (1.2 * max(-shipBBoxMin.x, shipBBoxMax.x), 1.2 * max(-shipBBoxMin.y, shipBBoxMax.y), 1.2 * max(-shipBBoxMin.z, shipBBoxMax.z))
        else:
            raise RuntimeError('StretchEffect: needs GetLocalBoundingBox')
        self.AddToScene(self.gfxModel)
