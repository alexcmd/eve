...
class GameUI(service.Service):
    ...
    def OnClusterShutdownInitiated(self, explanationLabel, when, duration):
        ...
        message = localization.GetByLabel(explanationLabel, intervalRemaining=localizationUtil.FormatTimeIntervalWritten(when - now, showTo=localizationUtil.TIME_CATEGORY_MINUTE), shutdownTime=util.FmtDate(when, 'ns'))
        if duration:
            message += localization.GetByLabel('/EVE-Universe/ClusterBroadcast/DowntimeMessageSuffix', downtimeInterval=localizationUtil.FormatTimeIntervalWritten(duration * const.MIN, showTo=localizationUtil.TIME_CATEGORY_MINUTE))
    ...
    def OnSessionChanged(self, isRemote, session, change):
        ...
        if session.charid is None:
            self._HandleUserLogonSessionChange(isRemote, session, change, viewSvc)
        elif session.stationid is not None:
            self._HandleSessionChangeInStation(isRemote, session, change, viewSvc)
        elif session.worldspaceid is not None:
            self._HandleSessionChangeInWorldSpace(isRemote, session, change, viewSvc)
        elif session.solarsystemid is not None:
            self._HandleSessionChangeInSpace(isRemote, session, change, viewSvc)
        else:
    ...
    def SetupViewStates(self):
        ...
        viewSvc.AddView('worldspace', viewstate.WorldspaceView())
        ...
        viewSvc.AddTransitions(('station', 'worldspace'), ('hangar', 'starmap', 'systemmap', 'planet', 'worldspace'), viewstate.FadeToBlackLiteTransition(fadeTimeMS=100))
        viewSvc.AddTransitions(('inflight', 'charsel', 'hangar', 'charactercreation', 'station', 'worldspace'), ('station', 'worldspace'), viewstate.FadeToCQTransition(fadeTimeMS=200, fallbackView='hangar', allowReopen=False))
        ...
        viewSvc.AddTransitions(('station', 'hangar', 'starmap', 'systemmap', 'worldspace'), ('charactercreation',), viewstate.FadeToBlackTransition(fadeTimeMS=200))
        viewSvc.AddTransition('charactercreation', ('station', 'worldspace'), viewstate.FadeFromCharRecustomToCQTransition(fadeTimeMS=250))
    ...
    def SetLanguage(self, key):
        ...
        if convertedKey in localization.GetLanguages() and key != prefs.languageID:
            ...
            localization.LoadSecondaryLanguageForClient(key)
    ...
    def CacheCameraTranslation(self):
        ...
        if camera is not None:
            setattr(self, 'cachedCameraTranslation', camera.translationFromParent)
    ...
    def _NewState(self, bp, hint):
        ...
        if bp and bp.balls.get(bp.ego, None):
            cachedCameraTranslation = getattr(self, 'cachedCameraTranslation', -1.0)
            sm.GetService('camera').LookAt(bp.ego, cachedCameraTranslation, smooth=False)
