#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/parklife/spaceMgr.py
import service
import uthread
import blue
import uix
    ...
    def DoBallsAdded_(self, ballsToAdd):
        ...
        for each in ballsToAdd:
            ...
            if slimItem is None:
                ...
                continue
            itemID = slimItem.itemID
            if slimItem.categoryID in (const.categoryCelestial, const.categoryStation) or util.IsUniverseCelestial(itemID):
                celestials.append(each)
            else:
                nonCelestials.append(each)

        ballsToAdd = nonCelestials
        for each in celestials:
            ball, slimItem, ob = each
            bp = sm.GetService('michelle').GetBallpark()
            if bp is None:
                return
            ...
            if slimItem is None:
                ...
                continue
            itemID = slimItem.itemID
            self.LogInfo('Preemptively loading celestial', itemID)
                ...
                if blue.os.GetWallclockTimeNow() > nextThreatCheck:
                    ...
                    for each in threatening:
                        ...
                        if slimItem is None:
                            ...
                            continue
                        itemID = slimItem.itemID
                        self.LogInfo('Preemptively loading threat', itemID)
                        self.LoadObject(ball, slimItem, ob)
                        numBallsAdded += 1
    ...
    def OnSpecialFX(self, shipID, moduleID, moduleTypeID, targetID, otherTypeID, area, guid, isOffensive, start, active, duration = -1, repeat = None, startTime = None, graphicInfo = None):
        ...
        if shipID == eve.session.shipid:
            if guid == 'effects.JumpOut':
                ...
                self.Indicate(localization.GetByLabel('UI/Inflight/Messages/Jumping'), '<center>' + localization.GetByLabel('UI/Inflight/Messages/NotifyJumpingToBeacon'))
