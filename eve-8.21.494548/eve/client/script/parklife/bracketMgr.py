...
class BracketMgr(service.Service):
    ...
    def Reload(self, *args, **kwds):
        self.CleanUp()
        if settings.user.ui.Get('showInTargetRange', True):
            self.inTargetRangeTimer = base.AutoTimer(1000, self.ShowInTargetRange)
        if settings.user.ui.Get('showBiggestDamageDealers', True):
    ...
    def SetupProps(self):
        ...
        self.grp_mapping = {const.groupCargoContainer: ('ui_38_16_12',
         ...
         const.groupStrategicCruiser: ('ui_38_16_2',
                                       ...
                                       0,
                                       0),
         const.groupAttackBattlecruiser: ('ui_38_16_2',
                                          1,
                                          0.0,
                                          inf,
                                          0,
         ...
         const.groupTransportShip: ('ui_38_16_3',
                                    ...
                                    0,
                                    0),
         const.groupBlockadeRunner: ('ui_38_16_3',
                                     1,
                                     0.0,
                                     inf,
                                     0,
         ...
         const.groupBeacon: ('ui_38_16_14',
                             ...
                             0,
                             0),
         const.groupSatellite: ('ui_38_16_14',
                                1,
                                0.0,
                                inf,
                                0,
    ...
    def IsWanted(self, ballID, typeID = None, groupID = None, categoryID = None, slimItem = None, filtered = None):
        if groupID in self.unwantedBracketGroups:
            ...
            return sm.GetService('tactical').WantIt(slimItem, filtered, isBracket=True)
    ...
    def AddBracketIfWanted(self, slimItem, ball = None):
        try:
            ...
            if not ball:
                return
            panel = self.GetNewBracket(itemID=itemID)
    ...
    @telemetry.ZONE_METHOD
    def GetNewBracket(self, itemID = ''):
        bracket = uicls.InSpaceBracket(parent=uicore.layer.bracket, name='__inflightbracket_%s' % itemID, align=uiconst.NOALIGN, state=uiconst.UI_NORMAL)
        return bracket
        ...
        for damage, bracketID in newDamageDealers:
            ...
            if bracket:
                ...
                if getattr(prefs, 'showDamageOnTarget', False):
                    ...
                    if not label:
                        label = bracket.debugLabel = uicls.EveLabelSmall(text='', parent=bracket, top=42, align=uiconst.CENTERTOP, state=uiconst.UI_DISABLED)
                    label.text = damage
            counter += 1
    ...
    def ShowInTargetRange(self, *args):
        ...
        for key, bracket in self.brackets.items():
            ...
            showCategory = settings.user.ui.Get(settingConfigName, True)
    ...
    def ShowModuleRange(self, moduleID, rangeDistance, *args):
        if not settings.user.ui.Get('showInTargetRange', True):
            return
        bp = sm.GetService('michelle').GetBallpark()
    ...
    def ShowHairlinesForModule(self, moduleID, reverse = False):
        if not settings.user.ui.Get('showModuleHairlines', True):
