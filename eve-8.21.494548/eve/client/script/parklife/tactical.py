...
class TacticalSvc(service.Service):
    ...
    def PrimePreset(self):
        ...
        if getattr(self, 'preset', None) is None or self.preset[OVERVIEW_CONFIGNAME] != self.activePreset:
    ...
    def GetGroups(self):
        self.PrimePreset()
        return self.preset[OVERVIEW_GROUPDATA]['groups']
    ...
    def GetBracketGroups(self):
        self.PrimePreset()
        return self.preset[BRACKETS_GROUPDATA]['groups']

    def GetValidGroups(self, isBracket = False):
        if isBracket:
            groups = set(self.GetBracketGroups())
        else:
            groups = set(self.GetGroups())
        return groups.intersection(self.GetAvailableGroups(getIds=True))

    def GetFilteredStates(self, isBracket = False):
        self.PrimePreset()
        if isBracket:
            return self.preset[BRACKETS_GROUPDATA].get('filteredStates', [])
        else:
            return self.preset[OVERVIEW_GROUPDATA].get('filteredStates', [])

    def GetEwarFiltered(self):
        self.PrimePreset()
        return self.preset[OVERVIEW_GROUPDATA].get('smartFilters', [])
    ...
    def ChangeSettings(self, what, value, add):
        ...
        presets = settings.user.overview.Get('overviewPresets', {})
        activePreset = self.preset[OVERVIEW_GROUPDATA].copy()
    ...
    def SetSettings(self, what, set):
        if what == 'groups':
            preset = self.preset[OVERVIEW_GROUPDATA].copy()
            preset['groups'] = set
            presets = settings.user.overview.Get('overviewPresets', {})
    ...
    def WantIt(self, slimItem, filtered = None, isBracket = False):
        if not self.preset[BRACKETS_CONFIGNAME] and isBracket:
            return True
        if self.logme:
    ...
    def IsEwarFiltered(self, ewarType):
        self.PrimePreset()
        ewarFilters = self.preset[OVERVIEW_GROUPDATA].get('smartFilters', [])
        return ewarType in ewarFilters

    def SmartFilter(self, itemID, overview = True):
        if overview:
            activePreset, preset = self.preset[OVERVIEW_CONFIGNAME], self.preset[OVERVIEW_GROUPDATA]
        else:
            activePreset, preset = self.preset[BRACKETS_CONFIGNAME], self.preset[BRACKETS_GROUPDATA]
